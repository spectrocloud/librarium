...
- **Reveal modal exposes a Connect Agent shortcut (unapplied).** The reveal modal has a **Connect Agent** button inline with the token cell that opens the Connect a coding agent dialog directly. Using this shortcut would save two navigation steps in Point Claude Code at the Appliance (no need to close the modal, no need to return to Overview and click Connect Coding Agent). Offer to the user; leave the current Overview-anchored flow in place until they decide.
