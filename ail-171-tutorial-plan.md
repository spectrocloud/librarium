...
- **Reveal modal Connect Agent shortcut adopted.** The reveal modal has a **Connect Agent** button inline with the token cell that opens the Connect a coding agent dialog directly. Restructured to use it: added step 10 to Create a Client and Its API Token (*"In the same modal, select **Connect Agent**"*), dropped the old *"Notice that your new client now appears on the Clients & API tokens page"* observation (weakened on already-populated appliances anyway), and removed the redundant Overview navigation from Point Claude Code at the Appliance step 1 (which used to duplicate the same nav from Download the Platform CA Certificate). Renumbered remaining steps. Applied.
