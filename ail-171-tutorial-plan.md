...
- **Routing step rewritten to map every row.** The live 1.1.5 Tier map has seven alias rows (`claude-fable-`, `claude-opus-`, `claude-sonnet-`, `claude-haiku-`, `gpt-`, `gemini-`, `grok-`), and the wizard blocks Next with *"Map every alias to a model to continue"* until every row is bound to a deployed model or the `Use Semantic Router rules` fallback. The tutorial's old three-row instruction left the reader stuck. Rewrote step 6 to instruct mapping every row to the deployed model. Dropped the "Claude Code asks by kind of work" pedagogy — that context can move to Next Steps if needed. Applied.
- **Product concern to flag with SME (unapplied).** Why must a Claude Code reader map `gpt-`, `gemini-`, `grok-`, and `claude-fable-` to complete the wizard? Claude Code only ever asks for `claude-opus-`, `claude-sonnet-`, and `claude-haiku-`. Either the tier map should accept partial mappings when the client's egress is `local-only` (the un-mapped aliases would naturally 404), or the wizard should default un-mapped aliases to `Use Semantic Router rules` on the reader's behalf. Not a tutorial fix — file with product.
- **Change Two Things and Watch collapsed to Change a Setting and Watch.** The sonnet-thinking experiment assumed the reader's model supports thinking. Observed on 1.1.5: the Thinking column reads *"Not supported by this model"* for every row when the row is mapped to `qwen3.6-35b-a3b-fp8`. Since Decision #2 keeps the tutorial from naming a specific model, the tutorial can't guarantee any model's thinking-capability. Dropped the sonnet-thinking experiment; kept the `CLAUDE_CODE_MAX_OUTPUT_TOKENS=1024` experiment (universal, model-agnostic). Renamed the H2 and reworded the section intro accordingly. Applied.
- **What You Built recap made state-neutral.** Was: *"You deployed a model onto your own GPU, ... routed three Claude aliases to that model ... You also learned to read GPU memory as a model loads..."*. Now: *"You created a client and an API token, routed the Tier map's aliases to a model on your appliance, and had Claude Code answer a coding question using that model. You also watched the requests and tokens land under your client on the Usage page."* Anchors dropped: the deploy step (already-deployed readers skipped it), the "three Claude aliases" count (we now route every row), and the GPU memory pedagogy (only fresh-path readers see Watch the Weights Load). Applied.
- **Create client button relabelled Create.** Live 1.1.5 wizard's final button reads just **Create**. Tutorial line 224 was **Create client**. Applied.
- **Add API token dialog: Expires is now required.** Live 1.1.5 dialog marks **Expires** with a red asterisk and its helper text reads *"Token stops working at the end of this day. Pick a day within the next year."* Tutorial line 220 said *"leave **Expires** blank"* — the reader would be stuck with a greyed **Add Token** button. Rewrote step 7 to *"select a date within the next year in **Expires**"*. Applied.
- **Copy button is an icon, not text.** Live 1.1.5 token-reveal modal shows the token in a monospace cell with an unlabeled copy icon; there is no button labeled "Copy". Tutorial line 227 said *"select **Copy**"*. Rewrote to *"select the copy icon next to it"* per the style guide's UI-elements-without-text guidance. Applied.
- **Reveal modal Connect Agent shortcut adopted.** The reveal modal has a **Connect Agent** button inline with the token cell. Clicking it expands the coding-agent setup **inline within the same modal** — no new dialog opens. Restructured to use it: added step 10 to Create a Client and Its API Token (*"In the same modal, select **Connect Agent**. The coding-agent setup expands inline."*), dropped the old *"Notice that your new client now appears on the Clients & API tokens page"* observation (weakened on already-populated appliances anyway), and removed the redundant Overview navigation from Point Claude Code at the Appliance step 1 (which used to duplicate the same nav from Download the Platform CA Certificate). Renumbered remaining steps. The Claude Code CLI tab is auto-selected when the setup expands, so step 1 (*"Select the Claude Code CLI tab..."*) becomes a safe-redundancy confirmation rather than a required click. Applied.
- **Directory context clarified for the terminal side.** A novice reasonably wondered *"what directory should my terminal be in?"* — the tutorial's terminal commands are all directory-agnostic for the general Q&A the tutorial asks. Added two reassurances: intro paragraph now ends with *"The terminal can be in any directory."*, and Point Claude Code at the Appliance step 3 appends *"Any directory works."* to *"In your terminal, paste the configuration."* — repeated at the moment the reader is likely to open the terminal for the biggest paste. Applied.
- **Point Claude Code at the Appliance step 1 and 2 collapsed.** The old step 1 (*"Select the Claude Code CLI tab, and then select your shell."*) had a safe-redundancy tab click (tab is auto-selected) and buried the shell picker's actual location (dropdown above the code block, not next to the tab). Merged shell selection with the copy step into a single, more specific instruction: *"Select your shell from the dropdown above the code block. Then select the copy button to copy the configuration."* Net change: 4 numbered steps → 3, and the tutorial's flow now maps cleanly onto the panel's own numbered instructions (panel step 1 is CA certificate, handled earlier; panel steps 2 and 3 map to tutorial steps 1–3; panel step 4 maps to the tutorial's `claude --print` verification block). Applied.
- **Token no longer copied manually.** The panel's step 3 line shows `export ANTHROPIC_AUTH_TOKEN=lpai_...` with a copy button that copies the whole line with the real token filled in — no substitution needed. Rewrote step 9 of Create a Client and Its API Token from *"select the copy icon next to it"* to *"The console reveals the token, which begins with `lpai_`"* — pure observation. Updated the warning admonition from *"Copy it now"* to *"Keep this modal open until you finish Point Claude Code at the Appliance"* — the underlying "shown once" safety concern is now the reason to keep the modal open, since the panel's copy button is the only way to move the token from screen to shell. Also rewrote step 3 of Point Claude Code from *"Replace `<per-user-token>` with the token you copied"* to *"select the copy button next to the `export ANTHROPIC_AUTH_TOKEN=...` line. In your terminal, paste it. The panel fills in your real token, so nothing needs replacing."* Removed the example bash block that showed the `<per-user-token>` placeholder — no longer relevant. Applied.
- **`claude --print CC_OK` verification dropped in favor of panel step 4.** The panel's step 4 (*"Run `claude`"*) is right there under step 3, so a reader would naturally follow it — bypassing the tutorial's `claude --print "reply with exactly CC_OK"` verification. Aligned: added step 4 in Point Claude Code (*"Start Claude Code. `claude`"*) matching the panel. The reader is now in an interactive session at the section boundary. Reworked Ask a Question, and Then Ask Another to continue in that session — dropped the redundant `claude` startup at the top, and moved *"That reply came from your own hardware"* from the CC_OK moment to right after the first coding-question answer. Also added *"In your Claude Code session"* to the question instruction as a mild reassurance that the session should still be alive. Applied.
- **Configuration-explanation paragraph and example env block dropped.** The paragraph after step 3 (*"The configuration sets your appliance address, the platform CA certificate you downloaded in **Download the Platform CA Certificate**, and the model alias for each tier..."*) plus its 9-line example bash block were post-hoc explanation of what the reader had already pasted, plus a copy-paste hazard (an illustrative block with `<appliance-address>` and generic model versions that a skimming reader could mistake for the real thing). Reader has the real config in their own terminal now; Claude Code Configuration reference is already in Next Steps for value-by-value explanation. Applied.
- **Ask a Question, and Then Ask Another converted to a numbered list.** The section had two prose paragraphs that each contained an action (ask a question / ask a second question) plus an observation. Numbered form is more Diátaxis-strict for multi-action sequences: reader sees two discrete beats, each with a clear "what to do" and "what to expect". *"That reply came from your own hardware"* stays as the payoff after step 1's answer arrives. Applied.
