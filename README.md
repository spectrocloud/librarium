...
We have an OpenAPI spec file (also known as Swagger files) for the Palette API.
...
```bash
npm run run-api-parser && docusaurus gen-api-docs palette
```
...
`docusaurus.config.js` file, specifically in the `plugin` property of the `config` object. 
