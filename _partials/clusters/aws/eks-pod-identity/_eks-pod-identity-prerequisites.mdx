...
- Self-hosted {props.edition} deployed on an Amazon EKS cluster with Kubernetes version 1.24 or later.

  - A ConfigMap named `palette-global-config` must exist in the `kube-system` namespace with a `managementClusterName` key set to the EKS management cluster's name where {props.edition} is deployed.
    ...
    1. Log in to the Local UI of the leader host of the {props.edition} management cluster.
    ...
    5. If the ConfigMap does not exist, use the following kubectl command to create it. Replace
       `<eks-management-cluster-name>` with the name of your EKS management cluster where {props.edition} is deployed.
...
- If you need your workload clusters to access AWS resources in the same AWS account as the EKS management cluster, three [IAM roles](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_create_for-user.html) must be created for
  {props.edition} and two of its services.
