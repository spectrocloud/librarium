...
- Registry credentials and TLS certificates are required for the Palette installation. You can choose to use either an
  internal Zot registry that comes with Palette or an external registry of your choice.
  ...
  Ensure that the Domain Name System (DNS) name listed in the certificate's Common Name (CN) and/or the Subject Alternative Name (SAN) resolves to the virtual IP address (VIP) that will be assigned to the Palette management cluster.

  </TabItem>

  <TabItem value="external-registry" label="External Registry">

  If you plan to use an external registry, you must provide the following external registry credentials during the Palette installation process:

  - The DNS/IP endpoint and port for the external registry.
  - The username for the registry.
  - The password for the registry.
  - The certificate for the registry used for TLS encryption in Base64 format.
  - The private key for the registry certificate used for TLS encryption in Base64 format.
  - (Optional) The Certificate Authority (CA) certificate that was used to sign the registry certificate in Base64 format.
