...
32. In **Cluster Config**, configure the following options.
    ...
    | **Virtual IP Address (VIP)**               | The virtual IP address for the cluster. This is used for load balancing and high availability. Not applicable if **Enable overlay** is set to true. | String   | _No default_ |
    | **Enable overlay (Optional)**              | Whether to enable the overlay network for the cluster. Refer to <VersionedLink text="Enable Overlay Network" url="/clusters/edge/networking/vxlan-overlay/"/> to learn more about overlay networks. | Boolean  | **False**     |
    | **Overlay CIDR range**                     | _Only applicable if **Enable overlay** is set to true._ The CIDR range for the overlay network. This is used to allocate IP addresses for the overlay network. Change the IP range to your requirements. | CIDR notation | _Palette attempts to allocate an unused IP range._ |
