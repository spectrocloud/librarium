...
- The following artifacts must be available in the root home directory of the RHEL airgap VM. You can download the files
  ...
  - {props.edition} installation binary.
...
14. Reboot your RHEL VM to apply the changes.
    ...
    the same commands from steps **5**, **6**, and **12** of this guide.
...
16. Update the SSL certificate file and key in the **httpd** service.
...
17. Start the {props.edition} installation binary, which uploads release-specific {props.edition} images and packs to the Harbor registry.
...
18. Grant the Apache user and group the necessary permissions to serve the files in the `/var/www/html` directory.
