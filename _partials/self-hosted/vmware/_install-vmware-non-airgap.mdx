...
partial_name: install-vmware-non-airgap
---
...
3.  Find the OVA download URL corresponding to your {props.version} version in the
    ...
    the Operating System and Kubernetes distribution OVA required for the install.
4.  Place the OVA in the `spectro-templates` folder. Refer to the
    ...
    Refer to the [Additional OVAs](../../../../../downloads/palette-vertex/additional-ovas.md) page for a list of
...
8.  Execute the `ec` command to launch the Palette CLI and begin the Enterprise cluster (EC) installation. The CLI walks
    ...
    the `ec` command, refer to [Palette Commands](../../../../../automation/palette-cli/commands/ec.md).
...
9.  Validate the installation.
    ...
    Refer to the [Validate Environment](../../../../../automation/palette-cli/commands/ec.md#validate-environment)
    section of the EC command for more information.

10. At the **Management Plane Type** prompt, choose **{props.version}**.

11. Select the desired OS you want to use for the installation. Review the table below for more information about each
    option.
    ...
    | **rhel** | Red Hat Enterprise Linux (RHEL).                                                          | RHEL subscription and a custom RHEL vSphere template with Kubernetes. Review the [RHEL and Palette eXtended Kubernetes](../../../../../byoos/image-builder/build-image-vmware/rhel-pxk.md) page to learn how to create the required template. |
...
15. Choose the image registry configuration. By default, our support team provides you with the credentials for the
    ...
    | **Kind Cluster Name**                            | Enter a name for the temporary Kubernetes cluster (Kind) that {props.version} uses during the installation process. This cluster runs inside the {props.version} VM and is used only for bootstrapping system components. The name must be lowercase, contain no spaces, and does not need to match your final management cluster name. |
...
16. The next set of prompts asks for the VMware vSphere account information. Enter the information listed in the table
    ...
    | **NTP servers**                                          | If required, rovide a list of Network Time Protocol (NTP) servers, such as `pool.ntp.org`, to ensure accurate time synchronization for the deployed VMs.                                                                                                                                             |
...
18. The last set of prompts are for the vSphere machine and database configuration. Use the following table for
    ...
    | **Parameter**     | **Description**                                                                            |
    | ----------------- | ------------------------------------------------------------------------------------------ |
    | **Node Affinity** | Select the node affinity. Enter `y` to schedule all [version] pods on control plane nodes. |
    ...
    If an error occurs during installation, restart it. After choosing the OS, the installer detects an existing `kind` cluster and display the following message.
    ...
    When the installation is complete, the terminal displays the EC cosole URL and default credentials. Save this information to access the [version] System Console. The Palette CLI has the kubectl CLI included. You can find the kubectl binary in the **bin** directory of the Palette CLI configuration
...
19. Log in to the system console using the credentials provided by the installer. After login,
    ...
    ![Screenshot of the {props.version} system console showing Username and Password fields.](/vertex_installation_install-on-vmware_vertex-system-console.webp)