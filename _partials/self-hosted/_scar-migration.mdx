...
6.  Select the **Pack Registries** tab, click the **three-dot Menu** at the end of the OCI registry row, and then select **Edit**.
...
11. Invoke the script to update the SCAR endpoint. Replace `<cluster-url>` with the address of your {props.edition} instance and
    ...
    You have now migrated SCAR to the OCI registry that hosts the {props.edition} images and packs.
