...
5. Upload the pack bundles to the OCI registry using the `oras` CLI tool. The following example commands upload a pack bundle
   ...
   registry can take approximately one hour. If the authentication token expires, you will need to re-authenticate to
   the OCI registry and restart the upload process.

   :::

6. Log in to the {props.version} system console.

7. From the left main menu, select **Administration**, and then select the **Pack Registries** tab.

8. Select the three-dot menu for your external registry and click **Sync**.
