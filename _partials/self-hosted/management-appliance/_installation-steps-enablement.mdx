...
1. Create a file called `user-data` using the following template and replace `<storage-drive>` with the drive to use for the {props.version} ISO stack, such as `/dev/sda`.
   ...
   If your nodes only support a single removable media connection, skip to step 5.
...
7. Attach the {props.iso} ISO and the user-data ISO to the nodes and ensure the boot order is set to boot from the {props.iso} ISO first. 
   ...
   If your nodes only support a single removable media connection, attach the {props.iso} ISO only.

   :::

8. Restart the nodes to start the installation process.

   If your nodes only support a single removable media connection, read and follow the additional steps below before restarting.
