...
5. Once the nodes have rebooted and entered the GRand Unified Bootloader (GRUB) menu, select the **Palette Edge Interactive Installer** option and press **ENTER**.
...
7. Issue the following command to edit the installation manifest.

   ```bash
   vi /oem/stylus_config.yaml
   ```

8. Add the following `install.device` section to your manifest, replacing `<storage-drive>` with the device name identified in step 6.
...
9. Issue the following command to start the installation process.
...
20. Log in to Local UI using the credentials you provided in step 10.
...
30. Repeat steps 27-31 for every host you want to link to the leader host.
