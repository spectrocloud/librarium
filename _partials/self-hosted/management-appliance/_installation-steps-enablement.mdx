...
1. Navigate to the [Artifact Studio](https://artifact-studio.spectrocloud.com/) and log in. Under **Install {props.iso}**, select the version to install and click **Show Artifacts**. Refer to the <VersionedLink text="Artifact Studio guide" url="/downloads/artifact-studio"/> for instructions on how to access and download artifacts.

   The following table describes the artifacts available for the {props.app}.

   | **Artifact**                          | **Description**                                                                                                    |
   | ------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
   | **Content bundle (including Ubuntu)** | Content bundle to pair with the slim **Appliance ISO**. Includes the OS content and {props.version} packs.         |
   ...
   Download the **Appliance ISO with Content**, or download both the **Appliance ISO** and **Content bundle (including Ubuntu)**. Download the **MOK Key for Secure Boot** if you use secure boot on your host.

   :::info

   Use the slim **Appliance ISO** with the **Content bundle (including Ubuntu)** when you have limited network capacity for mounting ISOs and greater network capacity to upload the content bundle after the initial install.
...
5. Once the nodes have rebooted and entered the GRand Unified Bootloader (GRUB) menu, allow {props.version} to select the **Palette Edge Interactive Installer** boot option automatically.
   ...
   as well. Wiping disks is optional and must be confirmed on the following screen. Carefully verify the selected disks before proceeding.

    :::

6. <PartialsComponent category="self-hosted" name="secure-boot-mokmanager" />

7. Once the **Palette Edge Interactive Installer** window appears, select the target disk for installation.
...
8. Choose the post-installation action: **nothing**, **reboot**, or **poweroff**, then press **ENTER**.

9. Review the **Installation Summary**.

10. Press **ENTER** to start the installation.

11. Wait for the installation process to complete. This will take at least 15 minutes, depending on the resources
...
12. In the Palette TUI, provide credentials for the initial account. This account will be used to log in to Local UI and for SSH access to the node.
...
13. In the Palette TUI, the available configuration options are displayed and are described in the next three steps.
...
14. In **Hostname**, check the existing hostname and, optionally, change it to a new one.

15. In **Network Adapter**, select a network adapter to configure. By default, the network adapters request an IP
    ...
    - You can change the IP addressing scheme of the adapter and choose static IP instead of DHCP. In Static IP mode,
      you need to provide a static IP address, subnet mask, and the address of the default gateway. Specifying
      a static IP removes the existing DHCP settings.

    - If desired, specify the VLAN ID. A VLAN ID enables you to logically segment network traffic on the same physical
      network interface, providing network isolation and enhanced traffic management. If you assign a VLAN ID, the
      host tags all outgoing packets from that adapter with the specified VLAN identifier.
...
16. In **DNS Configuration**, specify the IP address of the primary and alternate name servers. You can optionally
    specify a search domain.

17. After you are satisfied with the configurations, navigate to **Quit** and press **ENTER** to finish the
...
18. Ensure you complete the configuration on each node before proceeding to the next step.

19. Decide on the host that you plan to use as the leader of the group. Refer to <VersionedLink text="Link Hosts" url="/clusters/edge/local-ui/cluster-management/link-hosts#leader-hosts"/> for more information about leader hosts.

20. Access the Local UI of the leader host. Local UI is used to manage the {props.version} nodes and perform administrative
...
21. Log in to Local UI using the credentials you provided in step 11.

22. (Optional) If you need to configure a HTTP proxy server for the node, follow the steps in the <VersionedLink text="Configure HTTP-Proxy in Local UI" url="/clusters/edge/local-ui/host-management/configure-proxy"/> guide. When done, proceed to the next step.

23. From the left main menu, click **Linked Edge Hosts**.

24. Click **Generate token**. The host begins generating tokens that you will use to link this host with other
...
25. Click the **Copy** button to copy the token.

26. Log in to Local UI on the host that you want to link to the leader host.

27. From the left main menu, click **Linked Edge Hosts**.

28. Click **Link this device to another**.

29. In the pop-up box that appears, enter the token you copied from the leader host.

30. Click **Confirm**.

31. Repeat steps 25-29 for every host you want to link to the leader host.

32. Confirm that all linked hosts appear in the **Linked Edge Hosts** table. The following columns should show the
    required statuses.
    ...
    If you installed the {props.app} using the slim **Appliance ISO**, upload the **Content bundle (including Ubuntu)** to Local UI before proceeding. The **Content** column will not show **Synced** until the content bundle is uploaded and synchronized. Refer to <VersionedLink text="Upload Content Bundle with Local UI" url="/clusters/edge/local-ui/cluster-management/upload-content-bundle#upload-content-bundle-with-local-ui"/> or <VersionedLink text="Upload Content Bundle with Palette CLI" url="/clusters/edge/local-ui/cluster-management/upload-content-bundle#upload-bundle"/> for the upload steps.

    :::

    Content synchronization will take at least five minutes to complete, depending on your network resources.

33. On the left main menu, click **Cluster**.

34. Click **Create cluster**.

35. For **Basic Information**, provide a name for the cluster and optional tags in `key:value` format.

36. In **Cluster Profile**, the **Imported Applications preview** section displays the applications that are included
...
37. In **Profile Config**, configure the cluster profile settings to your requirements. Review the following tables for
    ...
    | **Image pull secret (Optional)** | The image pull secret provided by Spectro Cloud support, encoded in Base64, that authenticates image pulls from Spectro Cloud OCI registries. The value is masked in Local UI. Leave the field blank if your appliance is airgapped and all workload clusters use mirror registries or image swap, or if you plan to add the secret later through the system console. Refer to <PaletteVertexUrlMapper edition={props.edition} text="Configure Image Pull Secret" url="/system-management/configure-image-pull-secret" /> for more information. | Base64 encoded string | _No default_ |
    ...
    The {props.app} uses the Piraeus Container Storage Interface (CSI), which requires the Distributed Replicated Block Device (DRBD) kernel module on each node. The `drbd-module-loader` init container in the `linstor-satellite` pod loads this module using the mode configured by the `LB_HOW` environment variable in the `piraeus-operator` pack. Starting with {props.version} 4.9.14, the following modes are available.

    | **Value**                        | **Description**                                                                                                | **Requirements**                                                                                    |
    | -------------------------------- | -------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
    | **`shipped_modules`** _(Default)_ | Installs pre-built, pre-signed DRBD `.deb` packages from the container image.                                        | Compatible with Secure Boot.                                                                        |
    | **`compile`**                    | Builds the DRBD kernel module from source on the target node at install time using Dynamic Kernel Module Support (DKMS). | The OS image must include kernel headers that match the running kernel. Not compatible with Secure Boot. |

    :::warning

    Do not set `LB_HOW` to `compile` on nodes with Secure Boot enabled. Modules compiled on the node are unsigned, and the kernel refuses to load them. This causes the `drbd-module-loader` init container to fail and the {props.app} installation to stall.

    :::

    Most deployments should leave `LB_HOW` at the default value of `shipped_modules`. To override the value, edit the `piraeus-operator` pack YAML in the cluster profile before you deploy the cluster. Locate the `LinstorSatelliteConfiguration` custom resource and update the `drbd-module-loader` init container.
    ...
    For more information about DRBD module loading modes and the container image requirements for `compile` mode, refer to the [Piraeus CSI](/integrations/piraeus-csi) integration page.

38. Click **Next** when you are done.

39. In **Cluster Config**, configure the following options.
...
40. In **Node Config**, configure the following options.
...
41. Click **Next** when you are done.

42. In **Review**, check that your configuration is correct. If you need to make changes, click on any of the sections
...
43. Once the cluster is provisioned, access the {props.version} system console using the virtual IP address (VIP) you configured
...
44. You will be prompted to log in to {props.version} system console. Use `admin` as the username and `admin` as the password.
    You will be prompted to change the password after logging in.

45. In the **Account Info** window, provide the following information.
