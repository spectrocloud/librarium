...
- On the jump host, install the following packages. Skopeo mirrors the internal Zot registry, and `jq` parses the registry catalog response.
  ...
  sudo apt-get install -y skopeo jq apt-transport-https ca-certificates curl gnupg
  ```
...
1. On the jump host, create a working directory and change into it.
    ...
    ```shell
    mkdir -p palette-backup-restore && cd palette-backup-restore
    ```

2. Download the utility archive.

    ```shell
    curl -O https://software.spectrocloud.com/scripts/backup-restore/Palette-Backup-Restore-v4.9.0.tar.gz
    ```

3. Extract the archive. The scripts extract into the current directory.

    ```shell
    tar xvf Palette-Backup-Restore-v4.9.0.tar.gz
    ```
