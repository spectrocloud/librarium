...
Before you begin, ensure that you have iLO 5 access with privileges to launch the remote console and change BIOS settings. You also need the `MOK.der` certificate file on your local computer. Skip to step 4 if you have already downloaded the certificate file.
  ...
  3. Select the version corresponding to your {props.edition} installer. Then, select **Show Artifacts**. The artifact list appears.
  4. **Download** the **MOK Key for Secure Boot** file.
  ...
  13. Save your changes. Exit the menu and reboot the server.
...
Before you begin, you need the `MOK.der` certificate file on your local computer. Skip to step 4 if you have already downloaded the certificate file.
  ...
  14. Save your changes. Press **ESC** to return to **Authorized Signature Database (db)**.
  15. Select **View Entries**. The `MOK.der` file shows in the database as **DRBD Module Signing**.
  16. Save your changes. Exit the menu and reboot the server.
