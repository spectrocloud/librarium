...
- {props.version} can be installed on a single node or on three nodes. For production environments, we recommend that three nodes be provisioned in advance for the {props.version} installation. We recommended the following resources for each node. Refer to the <PaletteVertexUrlMapper edition={props.edition} install={props.install} text="Size Guidelines" url="/install/#size-guidelines" /> section for additional sizing information.
