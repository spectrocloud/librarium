...
- If you are using Proxmox Virtual Environment (VE) as the infrastructure provider for Palette, set the VM CPU type to `host`. The default setting in the Proxmox UI is usually different, and leaving it unchanged can lead to MongoDB compatibility issues. For more information on CPU types, refer to the [Proxmox VE documentation](https://pve.proxmox.com/pve-docs/chapter-qm.html#_cpu_type). 
