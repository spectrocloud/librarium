...
<!-- prettier-ignore-start --> Beginning with Palette version 4.6.35, a <VersionedLink text="Private Cloud Gateway (PCG)" url="/clusters/pcg"/> is required for <VersionedLink text="self-hosted Palette" url="/enterprise-version/enterprise-version"/> 
 and <VersionedLink text="VerteX" url="/vertex/vertex"/> installations if you need to configure 
 both an Azure Public Cloud and Azure US Government account. Users who require clusters with access to both Azure cloud environments should use a PCG to create an Azure cloud account for either Azure Public Cloud or Azure US Government.    
 
  If you do not configure a PCG, you must install two instances of Palette or VerteX: one for Azure Public Cloud clusters and one for Azure US Government clusters. <!-- prettier-ignore-end -->
