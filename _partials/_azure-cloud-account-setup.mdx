...
5. Fill out the following information, and click **Confirm** to complete the registration.
   ...
   | **Disable Properties**            | Prevent Palette and Palette VerteX from creating Azure Virtual Networks (VNets) and other network resources on your behalf for static placement deployments. If you enable this option, all users must manually specify a pre-existing VNet, subnets, and security groups when creating clusters. |
...
6. After providing the required values, select **Validate** to validate the combination of your **Tenant ID**, **Client ID**, and **Client Secret**. If the provided values are correct, the message **Credentials validated** is displayed.

7. Select **Confirm** to register your {props.cloud_account} cloud.
