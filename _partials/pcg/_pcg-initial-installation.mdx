...
5. If you want to configure your PCG to use a proxy network, complete the following fields, as appropriate.
    ...
    | **Parameter**                     | **Description**                                                                                                                                                                                                                                                                                                |
   | :-------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
   | **HTTPS Proxy**                   | (Optional) Leave blank unless using an HTTPS proxy. This setting propagates to all PCG nodes and all workload cluster nodes using the PCG. Example: `https://<username>:<password>@<proxy-ip>:<proxy-port>`.                                                                                                                  |
   | **HTTP Proxy**                    | (Optional) Leave blank unless using an HTTP proxy. This setting propagates to all PCG nodes and all workload cluster nodes using the PCG. Example: `http://<username>:<password>@<proxy-ip>:<proxy-port>`.                                                                                                                 |
   | **No Proxy**                      | (Optional) If using an HTTP or HTTPS proxy, provide a list of local network Classless Inter-Domain Routing (CIDR) addresses, hostnames, and domain names to exclude from being proxied. This setting propagates to all nodes to bypass the proxy server, as well as all workload cluster nodes using the PCG. Example if you have a self-hosted environment: `my.company.com,10.10.0.0/16`.                                     |
   | **Proxy CA Certificate Filepath** | (Optional) If using an HTTP or HTTPS proxy, provide the filepath of the proxy's Certificate Authority (CA) certificate on the installer host. The certificate is copied to each PCG node when deploying the PCG cluster, and the provided path is used on the PCG cluster nodes. The CA certificate file _must_ be named `ca.crt`. Example: `/usr/local/share/ca-certificates/ca.crt`. <br /><br /> **NOTE**: Proxy CA certificates are _not_ automatically propagated to workload clusters using the PCG; these certificates must be added at either the tenant level or cluster profile level in the OS layer. 
    ...
    Take the following approach to propagate proxy server CA certificates on a per-cluster basis.
      ...
      <br />
            ```yaml hideClipboard title="Example OS layer configuration" {1,9,13-16}
            kubeadmconfig:
              preKubeadmCommands:
