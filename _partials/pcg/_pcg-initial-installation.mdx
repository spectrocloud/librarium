...
5. If you want to configure your PCG to use a proxy network, complete the following fields, as appropriate.
   ...
   | **No Proxy**                      | (Optional) If using an HTTP or HTTPS proxy, provide a list of local network Classless Inter-Domain Routing (CIDR) addresses, hostnames, and domain names to exclude from being proxied. This setting propagates to all PCG nodes to bypass the proxy server, as well as all workload cluster nodes deployed using the PCG. Example if you have a self-hosted environment: `my.company.com,10.10.0.0/16`.                                     |
