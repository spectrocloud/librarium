...
- Launchpad for VMs Appliance supports Secure Boot for Dell servers with UEFI and Hewlett Packard Enterprise iLO5. 
  Learn how to configure and install Secure Boot for Launchpad for VMs Appliance below.
...
- A virtual IP address (VIP) must be available for the Launchpad for VMs management cluster. This is assigned during the Launchpad for VMs
  installation process and is used for load balancing and high availability. The VIP must be accessible to all nodes in
  the Launchpad for VMs management cluster.