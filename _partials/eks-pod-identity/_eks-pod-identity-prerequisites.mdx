...
- If you need your workload clusters to access AWS resources in the same AWS account as the EKS management cluster, three [IAM roles](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_create_for-user.html) must be created for
  Palette and two of its services. The following table lists the IAM roles that must be
  created.
...
- If you need your workload clusters to access AWS resources in different AWS accounts to the one where your EKS management cluster is deployed, cross-account IAM roles must be created for
  ...
  | Hubble service | `SpectroCloudHubbleCrossRole` | Same AWS account as EKS management cluster |
  | Identity service            | `SpectroCloudIdentityCrossRole`  | Same AWS account as EKS management cluster |
  ...
  - The following trust policy must be assigned to the IAM target role created for Palette. This
    ...
    - Replace `<role-name-for-identity-service-iam-cross-role>` with the name of the IAM role created for the identity service (for example, `SpectroCloudIdentityCrossRole`).
      ...
      "Statement": [
        {
          "Effect": "Allow",
          "Principal": {
            "AWS": "arn:aws:iam::<aws-management-cluster-account-id>:role/<role-name-for-identity-service-iam-cross-role>"
          },
  ...
  <TabItem label="Hubble service IAM cross role" value="hubble-service-iam-cross-role">

  - The following trust policy must be assigned to the IAM cross role created for Hubble service. This
    trust policy is the same as outlined in the
    [Amazon EKS documentation](https://docs.aws.amazon.com/eks/latest/userguide/pod-id-association.html#pod-id-association-create).
  ...
  - The following permissions policy must be assigned to the IAM cross role created for Hubble service.
  ...
  <TabItem label="Identity service IAM cross role" value="identity-service-iam-cross-role">

  You must create an IAM cross role for the Palette identity service in two stages.
  
  1. Create the IAM role in the AWS account where the EKS management cluster is deployed with the trust policy outlined in this tab.
  2. After creation, assign the permissions policy outlined in this tab to the IAM role.
  
  <br />

  :::info

  The IAM cross role ARN for the Palette identity service must be self-referenced in its own permissions policy. This means you must create the role first, then attach the permissions policy afterward.

  :::

  - The following trust policy must be assigned to the IAM cross role created for the identity service. This
    trust policy is the same as outlined in the
    [Amazon EKS documentation](https://docs.aws.amazon.com/eks/latest/userguide/pod-id-association.html#pod-id-association-create).
  ...
  - The following permissions policy must be assigned to the IAM cross role created for the identity service.

    - Replace `<target-aws-account-id>` with the AWS account ID where the workload cluster resources are located.
    - Replace `<role-name-for-palette-iam-cross-role>` with the name of the IAM role created for Palette in the target AWS account (for example, `SpectroCloudPaletteTargetRole`).
    - Replace `<aws-management-cluster-account-id>` with the AWS account ID where the EKS management cluster is deployed.
    - Replace `<role-name-for-identity-service-iam-cross-role>` with the name of this IAM role created for the identity service (for example, `SpectroCloudIdentityCrossRole`).
    ...
    ```json
    {
      ...
      "Statement": [
        ...
        },
        {
          ...
          "Resource": [
            "arn:aws:iam::<target-aws-account-id>:role/<role-name-for-palette-iam-cross-role>",
            "arn:aws:iam::<aws-management-cluster-account-id>:role/<role-name-for-identity-service-iam-cross-role>"
          ]
        }
      ]
    }
...
- The IAM roles created for the Palette Hubble service and Palette identity service must have pod identity associations
  ...
  - Replace `<hubble-service-iam-cross-role-name>` with the name of the IAM role created for the Palette Hubble service (for example, `SpectroCloudHubbleCrossRole`).
  - Replace `<target-aws-account-id>` with the AWS account ID where the workload cluster resources are located.
  ...
  --namespace hubble-system \
  --service-account spectro-hubble \
  --role-arn arn:aws:iam::<aws-management-cluster-account-id>:role/<hubble-service-iam-cross-role-name>
  ...
  - Replace `<eks-cluster-name>` with the name of your Amazon EKS Cluster.
  - Replace `<aws-management-cluster-account-id>` with the AWS account ID where your EKS management cluster is located.
  - Replace `<identity-service-iam-cross-role-name>` with the name of the IAM role created for the Palette identity service (for example, `SpectroCloudIdentityCrossRole`).
  ...
  --namespace palette-identity \
  --service-account palette-identity \
  --role-arn arn:aws:iam::<aws-management-cluster-account-id>:role/<identity-service-iam-cross-role-name>
  ```
