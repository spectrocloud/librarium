...
if [[ -n "${PATCH_COMPONENT_UPDATES:-}" ]]; then
  ...
  if confirm "Does this patch release add a new CanvOS or Palette CLI version, or both?" n; then
    ...
    COMPONENT_UPDATES=false
     ...
     any_patch_cli_sha_supplied; then
  COMPONENT_ANSWER_GIVEN=true
  COMPONENT_UPDATES=true
else
...
if [[ "$COMPONENT_UPDATES" == false ]]; then
  ...
  echo "ℹ️  The GitHub CLI is not set up, so $NICKFURY_REPO cannot be read. Supply the component versions at the prompts below, or install gh and run 'gh auth login' to look them up." >&2
fi
...
for spec in "${PALETTE_CLI_ARCHES[@]}"; do
  ...
  if [[ "$PALETTE_CLI_CHANGED" == true ]]; then
    ...
    # release, and to this Palette CLI version, because a checksum recorded against a version this
    # run has moved away from belongs to a different binary.
    if [[ -z "$arch_sha" ]]; then
...
if [[ "$COMPONENT_UPDATES" == true && "$PALETTE_CLI_CHANGED" == false ]] &&
  ...
  NEWEST_DOCUMENTED_CLI=$(get_documented_table_version \
    "$DOWNLOADS_FILE" "$DOWNLOADS_CLI_VERSION_COLUMN" "$RELEASE_PATCH" "palette-cli-version-table" "any")
