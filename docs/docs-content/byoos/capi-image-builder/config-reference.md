...
| `containerd_version` | The containerd version to use when building the image. The version specified is pulled from the [containerd GitHub repository](https://github.com/containerd/containerd/). If using `containerd_url` in the [Airgap Configuration](#airgap-configuration) section, set `containerd_version` to the containerd version used in `containerd_url`. | Yes      |
| `crictl_version`     | The crictl version to use when building the image. The version specified is pulled from the [cri-tools GitHub repository](https://github.com/kubernetes-sigs/cri-tools). If using `crictl_url` in the [Airgap Configuration](#airgap-configuration) section, set `crictl_version` to the crictl version used in `crictl_url`.                   | Yes      |
...
| `vcenter_template`      | The vCenter path to an existing OVF template to use when building the image. If using this field, leave the [ISO Configuration](#iso-configuration) values blank. | No       |
...
| `k8s_rpm_key`       | The URL to the GPG key for the Kubernetes RPM repository. Used to verify package signatures when installing Kubernetes packages on RHEL-based systems. Overrides the default key from pkgs.k8s.io.                                                                                                                                                                                                                                                                                       | No       |
| `k8s_rpm_server`    | The URL to the Kubernetes RPM repository. Used to install kubelet, kubeadm, and kubectl packages on RHEL-based systems. Overrides the default repository at pkgs.k8s.io.                                                                                                                                                                                                                                                                                                                 | No       |
