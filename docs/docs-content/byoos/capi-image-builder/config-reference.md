...
| `airgap_ip`         | The IP address or hostname of the airgap support VM that has the required dependencies. Refer to the appropriate [Self-Hosted Palette](../../self-hosted-setup/palette/supported-environments/vmware/setup/airgap/airgap.md) or [Vertex](../../self-hosted-setup/vertex/supported-environments/vmware/setup/airgap/airgap.md) environment setup page for instructions on how to deploy an airgap support VM using either an OVA or RHEL VM. | Yes      |
