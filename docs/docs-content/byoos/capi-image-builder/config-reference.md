...
| `cni_version`        | The Container Network Interface (CNI) version to use when building the image. For airgap builds, the `cni_version` must match the CNI version in the Yum repo. Example: `kubernetes-cni-<version>-1.el8.x86_64.rpm `.                                                                                                                           | Yes      |
| `containerd_version` | The containerd version to use when building the image. The version specified is pulled from the [containerd GitHub repository](https://github.com/containerd/containerd/). For airgap builds, the `containerd_version` must match the `cri-containerd-cni` version in the Yum repo. Example: `cri-containerd-cni-<version>-linux-amd64.tar.gz`. | Yes      |
| `crictl_version`     | The crictl version to use when building the image. The version specified is pulled from the [cri-tools GitHub repository](https://github.com/kubernetes-sigs/cri-tools). For airgap builds, the `crictl_version` must match the crictl version in the Yum repo. Example: `crictl-v<version>-linux-amd64.tar.gz`.                                | Yes      |
