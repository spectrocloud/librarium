...
9.  Issue the command below to start the CAPI Image Builder container and assign the container ID to the `BUILD_ID`
    ...
    Before issuing the following command, replace `ubuntu` with your Linux username and `4.6.b` to match your CAPI Image Builder version.
    ...
    <!-- prettier-ignore -->
        ...
        3.  Issue the command below to start the CAPI Image Builder container and assign the container ID to the `BUILD_ID`
            ...
            your VMware vSphere environment. Replace `ubuntu` with your Linux username and `4.6.b` with the CAPI Image Builder version downloaded in step 1.
...
14. In the **Deploy OVF Template** wizard, select **Local File > Upload Files**, and choose the OVA file located in the
    `output` folder on your local machine. This guide uses `rhel-8-kube-v1.30.4.ova` as an example. Select **Next** to continue.
