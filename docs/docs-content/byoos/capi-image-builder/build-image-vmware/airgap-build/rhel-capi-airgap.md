...
- The following artifacts must be available in the root home directory of the airgap support VM. You can download the
  ...
  - [RHEL ISO](https://developers.redhat.com/products/rhel/download?source=sso) version `8.8`. Ensure you download the `x86_64-dvd.iso` file and not the `x86_64-boot.iso` file, and make sure you have its SHA256 checksum available. This
  ...
  - (Optional) Any custom Bash scripts (`.sh` or `.bash` files) that you want to execute when creating your RHEL image. Custom scripts are supported beginning with CAPI Image Builder version `4.6.b`.
...
3.  Ensure all artifacts listed in the [Prerequisites](#prerequisites) section are available in the `root` home
    ...
    ```text hideClipboard
    airgap-pack-kubernetes-1.30.5.bin  bin  capi-image-builder-v4.6.b.tgz  prep  rhel-8.8-x86_64-dvd.iso  snap
    ```
...
5.  Update the permissions of the `output` folder to allow the CAPI Image Builder tool to create directories and files within it.
...
17. Issue the command below to start the CAPI Image Builder container and assign the container ID to the `BUILD_ID_CAPI`
    ...
    Before issuing the following command, replace `4.6.b` to match your CAPI Image Builder version.
        ...
        ```bash
        BUILD_ID_CAPI=$(docker run --net=host --volume /root/output:/home/imagebuilder/output --detach us-docker.pkg.dev/palette-images/palette/imagebuilder/capi-builder:v4.6.b)
        ```
        ...
        ```bash
        BUILD_ID_CAPI=$(podman run --net=host --volume /root/output:/home/imagebuilder/output --detach us-docker.pkg.dev/palette-images/palette/imagebuilder/capi-builder:v4.6.b)
        ```
...
19. Once the build is complete, the RHEL 8 CAPI image will be downloaded to the `output` directory as the `image_name` specified in the `imageconfig` file. For this example, the image is `rhel-8`. Once the image is created, the VM is deleted from VMware vSphere. 
...
20. Copy the `rhel-8-kube-v1.30.5.ova` file to the home directory of the airgap support VM. Replace `ubuntu` with your airgap support VM username.

    ```shell
    cp /root/output/rhel-8/rhel-8-kube-v1.30.5.ova /home/ubuntu/
    ```

    Next, open a new terminal window on your local machine and use the `scp` command to copy the
    `rhel-8-kube-v1.30.5.ova` file. Replace `/path/to/private_key` with the path to the private SSH key, `ubuntu` with your airgap support VM username, and `palette.example.com` with the IP address or hostname of the airgap support VM.

    ```shell
    scp -i /path/to/private_key ubuntu@palette.example.com:/home/ubuntu/rhel-8-kube-v1.30.5.ova .
    ```
...
23. In the **Deploy OVF Template** wizard, select **Local File > Upload Files**, and choose the OVA file located in the
    `output` folder on your local machine. This guide uses `rhel-8-kube-v1.30.5.ova` as an example. Select **Next** to continue.
