...
[Rocky Linux](https://rockylinux.org/) image with <VersionedLink text="Palette eXtended Kubernetes (PXK)" url="/integrations/packs/?pack=kubernetes" /> for clusters that target VMware vSphere.
...
9.  Open the **imageconfig** template file located in the `output` folder and fill in the required parameters. For a
    ...
    VM hostname in the `airgap_ip` parameter. The same applies if you used the VM’s IP address.
...
16. Start the CAPI Image Builder container and assign the container ID to the `BUILD_ID_CAPI` variable. The tool will
    ...
    environment. It will then generate a Rocky 8 CAPI image from the VM and save it to the `output` folder.
...
18. Once the build is complete, the Rocky 8 CAPI image will be downloaded to a folder named `rocky-8` within the output
...
19. Copy the `rockylinux-8-kube-v1.28.9.ova` file to the home directory of the airgap support VM.
    ...
    ```shell
    scp -i /path/to/private_key ubuntu@palette.example.com:/home/ubuntu/rockylinux-8-kube-v1.28.9.ova .
    ```
...
21. Right-click the folder and select **Deploy OVF Template** to deploy a VM using the Rocky 8 OVA file that was built
    in step **16** of this guide.

22. In the **Deploy OVF Template** wizard, click **Local File**, then **Upload Files**, and select the
    `rockylinux-8-kube-v1.28.9.ova` file from the folder on your local machine. Click **Next** to continue.

23. Assign a name to the virtual machine, such as `rockylinux-8-kube-v1.28.9`, and select the folder you created
...
27. Once the VM is created, right-click it and select **Convert to Template**. This will convert the VM into a Rocky 8
