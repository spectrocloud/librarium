...
cluster is deployed by attaching it to the template. Once attached, the template governs the
cluster's future upgrades and policy enforcement in the same way as clusters originally deployed from the template.
...
6. Review any differences between the cluster's current configuration and the template. You must review all differences before proceeding.
...
To trigger the profile swap immediately instead of waiting for the maintenance window, use **Actions > Upgrade now** on
the template's **Overview** tab. Refer to [Modify Cluster Templates](./modify-cluster-templates.md#overview-tab) for
details.

:::

While the cluster is attached and waiting for or undergoing reconciliation, the following restrictions apply, matching
the behavior of clusters originally deployed with a template:
