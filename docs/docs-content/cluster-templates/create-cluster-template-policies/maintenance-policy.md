...
7.  Use the **Schedule** drop-down menu to specify when to initiate cluster updates. Update schedules are defined by
    cron expressions. Available options include the following:

    - **Every week on Sunday at midnight** (`0 0 * * SUN`)
    - **Every two weeks at midnight** (`0 0 1,15 * *`)
    - **Every month on the 1st at midnight** (`0 0 1 * *`)
    - **Every two months on the 1st at midnight** (`0 0 1 */2 *`)
    - **Custom**
...
8.  Select how many hours you want the **Upgrade window** to last. Values range from 1 - 24 hours. Updates will be
    deployed to clusters gradually during the upgrade window.
