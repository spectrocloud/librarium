...
- You can now pass through arbitrary CAPI and CAPx provider properties directly to cluster and node-level configurations
  ...
  Palette needing to maintain a static list of supported fields. Misconfigurations surface as clear, repeatable cluster
  events with user-friendly error messages that identify the offending passthrough properties. Day 2 updates that
  include passthrough changes display a blanket repave warning. Support is available across the UI, API, Terraform, and
  Crossplane for both Palette and Vertex.
...
- Fixed a secureboot install regression in the piraeus-operator pack where the DRBD kernel module was being built on the
  ...
  install. The DRBD module is now reverted to `shipped_modules` mode, shipping as pre-built, signed `.deb` files inside
  the image so secureboot-enabled hosts can load them correctly.
