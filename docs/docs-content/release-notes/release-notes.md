...
- Edge clusters in appliance mode can now use systemd extensions to deliver Kubernetes and Palette Agent binaries at
  ...
  them through systemd extensions. Set `system.uri: NA` in the BYOOS pack for standard upgrades. The new
  `BUNDLE_K8S_AND_AGENT_PROVIDER` flag in the CanvOS `.arg` file overrides the default when a specific flow requires the
  binaries embedded. Unified Kernel Image (UKI) deployments do not support systemd extensions and continue to receive
  these binaries embedded in the provider image. Refer to
