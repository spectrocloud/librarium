...
## August 12, 2026 - Release 4.9.x

<!-- PATCH RELEASE TICKET: DOC-3099 -->

### Improvements

<!-- https://spectrocloud.atlassian.net/browse/PLT-2325 -->

- Automated Reach Manifests version updates in `palette-cli` to reduce manual overhead.
...
- Fixed Longhorn pods entering `ImagePullBackOff` on edge appliances caused by a registry hostname alias mismatch
  between `docker.io` and `index.docker.io` in containerd 2.1.4, which prevented local image resolution and blocked CSI
  storage deployment.
...
<!-- https://spectrocloud.atlassian.net/browse/PLT-2331 -->

- Fixed incorrect image tags for `kube-rbac-proxy`, `spectro-reach`, and `cert-manager-acmesolver` in the `images.list`
  generated by the `palette-cli` periodic job.
