...
- You can now pass through arbitrary CAPI and CAPx provider properties directly to cluster and node-level configurations
  for EKS and AKS clusters. This allows advanced users to override provider-specific settings without waiting for native
  Palette support. Passthrough properties are applied at both Day 0 and Day 2, with a blanket repave warning on Day 2
  updates. Clear cluster events surface user-friendly error messages when a misconfiguration is detected, helping you
  trace and resolve issues quickly. This feature is supported across UI, API, Terraform, and Crossplane for both Palette
  and Vertex.

<!-- https://spectrocloud.atlassian.net/browse/PLT-2232 -->

- You can now configure SSH keys for MAAS cluster nodes (bare metal and LXD VMs) through the Palette UI, API, Terraform,
  and Crossplane. SSH keys are injected during cluster provisioning via cloud-init (Day 0) and can be rotated and
  managed by the cluster-management-agent (Day 2). Multiple SSH keys can be configured per cluster, and keys are
  validated for proper format before cluster creation. This feature is supported for both Palette and Vertex.
...
- Fixed a secureboot install regression in the piraeus-operator pack where the DRBD kernel module was built on the
  ...
  load. The module is now shipped as a pre-built, signed image (shipped_modules mode), allowing installations on
  hardened, STIG, and FIPS infrastructure to complete successfully.
...
- Fixed an issue in the `spectrocloud_cluster_edge_native` Terraform provider where importing an existing Edge Native
  cluster did not correctly read the `cloud_config.vip` value from Palette into Terraform state. This caused
  `terraform plan` to report false drift, showing the VIP as a pending addition even when the Terraform configuration
  and Palette already matched.
