...
## June 28, 2026 - Release 4.9.b {#release-notes-4-9-b}
...
- Connected Edge Native clusters now support upgrading the control plane independently from worker pools. Enable the
  **Skip worker node update** toggle on a worker pool to defer its Kubernetes upgrade while the control plane advances.
  ...
  prevent unsupported drift between the control plane and worker nodes. This enables smoother CK8s LTS upgrade
  orchestration on edge without repaving every worker pool on each profile bump. The toggle is available in both Palette
