...
- You can now use Generation 2 Azure VMs for AKS cluster nodes in Palette. Gen 2 VMs offer UEFI-based boot with Secure
  ...
  a Gen 2 variant, including Azure Linux 3 and Ubuntu. Day 2 changes that switch VM generation trigger a repave warning.
  Coverage is available across the UI, API, Terraform, and Crossplane for both Palette and Vertex.
...
- You can now pass through arbitrary CAPI and CAPx provider properties directly to cluster and node-level configurations
  ...
  Palette support. Misconfigurations surface as clear, repeatable cluster events that identify the offending passthrough
  properties. Day 2 updates that include passthrough changes display a blanket repave warning. Support is available
  across the UI, API, Terraform, and Crossplane for both Palette and Vertex.
...
- You can now configure SSH keys for MAAS cluster nodes, including bare metal machines and LXD VMs, through the Palette
  ...
  be rotated and updated by the cluster-management-agent (Day 2). Multiple SSH keys can be configured per cluster. This
  feature is supported for both Palette and Vertex.
...
- Fixed a secureboot install regression in the piraeus-operator pack where the DRBD kernel module was built on the
  ...
  now shipped as a pre-built, signed image (shipped_modules mode), restoring successful appliance installs on hardened,
  STIG, and FIPS infrastructure.

<!-- https://spectrocloud.atlassian.net/browse/PLT-2242 -->

- Fixed a Terraform provider issue where importing an existing Edge Native cluster did not correctly read the
  `cloud_config.vip` value from Palette into Terraform state, causing `terraform plan` to report false drift and show
  the VIP as a pending addition even when the configuration and Palette already matched.
