...
- You can now use Generation 2 Azure VMs for AKS cluster nodes in Palette. Gen 2 VMs offer UEFI-based boot, Secure Boot
  ...
  2 variant, including Azure Linux 3 and Ubuntu. Upgrading from Gen 1 to Gen 2 on an existing cluster triggers a node
  repave, and Palette displays the appropriate repave warning. Coverage is available across the UI, API, Terraform, and
...
- You can now pass through arbitrary CAPI and CAPx provider properties directly to cluster and node-level configurations
  for EKS and AKS clusters. This allows you to override provider-specific settings that are not natively exposed in
  Palette without waiting for a product update. Passthrough properties are applied on Day 0 and Day 2; Day 2 changes
  trigger a blanket repave warning. Misconfiguration errors are surfaced as clear, repeatable cluster events that
  identify the offending properties without exposing sensitive data. Support is available across the UI, API, Terraform,
...
- You can now configure SSH keys for MAAS cluster nodes (bare metal and LXD VMs) through the Palette UI, API, Terraform,
  and Crossplane. SSH keys are injected into all control plane and worker nodes during provisioning via cloud-init
  (Day 0) and can be rotated or updated by the cluster-management-agent (Day 2). Multiple SSH keys can be configured per
  cluster. This feature is supported for both Palette and Vertex.
...
- Fixed a secureboot install regression in the piraeus-operator pack where the DRBD kernel module was built on the
  ...
  now shipped as a pre-built, signed image (reverting from `compile` mode back to `shipped_modules` mode), allowing
  installation to complete successfully on hardened, STIG, and FIPS infrastructure.

<!-- https://spectrocloud.atlassian.net/browse/PLT-2242 -->

- Fixed an issue in the Terraform provider where importing an existing Edge Native cluster did not correctly read the
  `cloud_config.vip` value from Palette into Terraform state. After import, `terraform plan` incorrectly reported the
  VIP as a pending addition even when the value was already configured in both the Terraform module and the Palette UI.
  The import and read path now correctly reconciles the VIP, eliminating false drift for
  `spectrocloud_cluster_edge_native` resources.
