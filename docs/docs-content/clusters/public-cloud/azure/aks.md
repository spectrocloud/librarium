...
- A Secure Shell (SSH) key that you have pre-configured in your Azure environment. Only RSA SSH keys are supported for
  Azure AKS clusters deployed using Palette. Refer to the [SSH Keys](../../cluster-management/ssh/ssh-keys.md) guide for
  more information about creating and managing SSH keys in Palette.
