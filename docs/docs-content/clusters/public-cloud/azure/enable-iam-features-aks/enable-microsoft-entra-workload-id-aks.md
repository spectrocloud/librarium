...
Azure Kubernetes Service (AKS) clusters. This allows you to leverage Microsoft Entra Workload ID for secure
authentication of applications hosted on your AKS clusters.
...
- Azure CLI installed and configured on your local machine. Refer to the
  ...
  - You will need a minimum of read permissions on the cluster to be able to validate the feature enablement.

## Enable Microsoft Entra Workload ID

1. Log in to [Palette](https://console.palette.com).

2. In the left main menu, click on **Profiles**.
...
3. Click on the cluster profile you want to edit or
   ...
   - If creating a new cluster profile, select the **Azure AKS** on the Cloud Type page, and add your profile layers as
     needed. Create the cluster profile and then click on it from the **Profiles** page to edit the profile.

4. Click on the **Kubernetes** layer to edit the pack parameters.
...
2. In the left main menu, click **Clusters**.
