...
Clusters are launched from within Projects in Palette, and they belong to a single project. In the **Project** view, all
clusters that are launched from that project are listed for users with the Project Administrator role or Cluster
Administrator role. A Tenant Administrator can get an aggregated view of clusters running across all projects from the
**Organization** view, as follows:

1.  Log in to the **Palette Management Console** as a Tenant Administrator.

2.  Go to the **Clusters** option from the sidebar to list all the clusters belonging to all the users under that Tenant
    Administrator.
