...
* **Worker node pool repave**: This happens when changes to a node pool's specifications render the existing nodes incompatible with the pool's criteria. For instance, changing the hardware specifications of a worker pool. Nodes within the affected pool are sequentially replaced with new nodes that meet the updated specifications.
