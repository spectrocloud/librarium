...
worker nodes on an AWS IaaS, Azure IaaS, GCP IaaS, MAAS, VMware vSphere, Apache CloudStack, or connected Edge Native
cluster, you can manually trigger a worker node upgrade at any time after updating the control plane.
...
- An active AWS IaaS, Azure IaaS, GCP IaaS, MAAS, VMware vSphere, Apache CloudStack, or connected Edge Native cluster in
  Palette with the **Skip worker node update (Optional)** option enabled.
...
3. Locate and select the cluster you want to upgrade.

4. Select the **Nodes** tab to view the cluster's node pools.

5. Identify the worker node pool that has the **Skip worker node update (Optional)** option enabled and select the
   **Edit** button next to it.

6. In the **Edit node pool** dialog, disable the **Skip worker node update (Optional)** option.

7. **Confirm** your changes. In the dialog window, select **Continue** to proceed with the node pool repave. Once
   confirmed, Palette schedules the worker node upgrade.
...
4. Confirm that the **K8s Version** column for the updated worker node pools matches the control plane pool's Kubernetes
   version.
...
To remove unused resources created by a pack from a cluster, you need to manually delete the pack from the cluster
profile and apply the changes to the cluster. This will effectively remove all resources associated with the pack.
However, this approach may not always be desirable, and there are alternative solutions.

For example, you can use `kubectl` to scale the resource replicas to zero. While manually deleting the resource is an
option, Palette will automatically recreate it if the pack remains in the cluster profile. Scaling the replicas to zero
using `kubectl` ensures that no active instances of the resource are running, without triggering Palette to restore
them.
...
2. Create a cluster profile with two versions for an AWS IaaS cluster that includes the Hello Universe pack. For
   example:
...
3. Build a cluster using cluster profile version 1.0.0. This will take 15-20 minutes.

4. Download the [Admin kubeconfig](./kubeconfig.md) file from the cluster **Overview** tab and export the `KUBECONFIG`
   environment variable to the file path of the kubeconfig file.
