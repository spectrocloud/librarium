...
- If your Edge host is built with Palette Agent version 4.7.c-canvos or later, the `stylus.site.remoteShell.disable` field in
  the `user-data` file must be omitted or set to `false` before the Edge host registers with Palette. `false` is the
