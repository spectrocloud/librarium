...
| `stylus.site.remoteShell.disable` | When set to `true`, disables remote shell access to the host. This value can be modified only before the Edge host registers with Palette and cannot be changed afterward.                                                                                  | boolean                          | `false` |
