...
| `DisableWorkerNodeCapReconcile` | Prevents the Palette Edge node agent from reconciling the control-plane taint on control plane nodes when `UseControlPlaneAsWorker` is disabled. Use this feature gate if you want to manually remove the control-plane taint and prevent the Palette Edge node agent from adding it again. |
