...
| `name` | Hostname of the Edge device. A hostname is composed of a series of labels concatenated with dots. Each label must be from 1 to 63 octets long. The entire hostname, including the delimiting dots, has a maximum of 253 ASCII characters. Each labels may contain only the lower-case ASCII letters a through z, the digits 0 through 9, and the hyphen-minus character ('-'). A hostname must start and end with alphanumeric characters. |
