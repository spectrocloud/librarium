...
| `stylus.site.remoteShell.disable` | When set to `true`, disables remote shell access to the host. This value can only be modified before the Edge host registers with Palette and cannot be changed afterward. The field is supported for Edge hosts built with Palette agent version 4.7.c or later. | boolean                          | `false` |
