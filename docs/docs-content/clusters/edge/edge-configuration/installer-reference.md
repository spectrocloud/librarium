...
| `stylus.disableTui`                                         | Disables the ability to interact with the Palette Terminal User Interface (TUI), preventing users from modifying Edge host and network settings. Edge host information is still displayed when set to `true`. This flag cannot be changed post-installation.                                                                                                                                           | boolean | `false`   |
...
| `stylus.path`                                               | Specifies the Palette Edge node agent installation directory. The Palette Edge node agent appends its internal layout, `/opt/spectrocloud`, to this path. If you omit this parameter, the system uses `/` as the default root and installs the Palette Edge node agent to `/opt/spectrocloud`.                                                                                                         | String  | `/`       |
...
| `stylus.vip.skip`                                           | When set to `true`, the installer skips the configuration of kube-vip and enables the use of an external load balancer instead. <br /> <br /> **NOTE:** If the virtual IP (VIP) address assigned to the cluster during cluster creation is the same as the Edge host's IP address (for example, in the case of single-node clusters), kube-vip is disabled automatically.                              | boolean | `false`   |
...
| `stylus.site.ntpServers`                   | A list of NTP servers the host can synchronize internal clocks to UTC                                                                                                                                                                                       | String                           | `''`    |
...
| `stylus.site.remoteShell.disable`          | When set to `true`, disables remote shell access to the host. This parameter is supported for Edge hosts built with Palette agent version 4.7.16 or later.                                                                                                  | boolean                          | `false` |
...
| `stylus.site.users`                        | The list of users to create on the Edge host as part of site configuration. Each list item accepts parameters as follows in this table.                                                                                                                     | None                             |
| `stylus.site.users[*].groups`              | The list of groups that the user belongs to. Replace `*` with your username.                                                                                                                                                                                | None                             |
| `stylus.site.users[*].passwd`              | The password of the user. Replace `*` with your username.                                                                                                                                                                                                   | None                             |
| `stylus.site.users[*].passwordExpiry`      | The date in YYYY-MM-DD format when the password expires. Replace `*` with your username.                                                                                                                                                                    | None                             |
| `stylus.site.users[*].lock_passwd`         | When set to `true`, disables password configuration by the user. Replace `*` with your username.                                                                                                                                                            | `false`                          |
| `stylus.site.users[*].ssh_authorized_keys` | The list of public SSH keys authorized for the user. Replace `*` with your username.                                                                                                                                                                        | None                             |
