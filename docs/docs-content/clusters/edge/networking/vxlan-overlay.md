...
- All Edge hosts must be on the same Layer-2 network.
  - Broadcast messages must be allowed between all Edge hosts participating in the cluster.
    - Features such as promiscuous mode for VM hosts on VMware must be enabled to allow broadcasts between hosts.
    - Switches must not implement features that block broadcast between ports where Edge hosts are connected.
