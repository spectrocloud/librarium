...
- Depending on the console environment, the TUI may not display the full QR code for Palette registration. This issue
  ...
  partially truncated.
- In Agent mode, modifying VLAN settings using the TUI is allowed only if the settings were created via the TUI. Pre-existing settings are read-only. If you attempt to update pre-existing network
  settings via the TUI, a new configuration is created alongside the existing one. To manage
  pre-existing configurations, use the original configuration method such as `systemd-networkd`, Netplan, or
  NetworkManager.
