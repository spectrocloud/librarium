...
- Depending on the console environment, the TUI may not display the full QR code for Palette registration. This issue has been observed in the VMware vSphere web console, where limited console window size can cause the QR code to appear partially truncated.
