...
10. In **NTP Configuration**, specify one or more NTP servers (for example, 0.pool.ntp.org,1.pool.ntp.org).
   ...
   If cluster-wide NTP settings are later configured or updated through cluster settings, those values overwrite the host-specific configuration on all hosts in the cluster.
   :::
