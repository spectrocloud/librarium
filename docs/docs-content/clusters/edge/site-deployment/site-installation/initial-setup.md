...
1. Power up the Edge host. The GRand Unified Bootloader (GRUB) screen will display the following options. Do not make
   ...
   | Palette eXtended Kubernetes - Edge Registration | If the Edge host is already part of a cluster, this option will not appear. This option boots the Edge host to the registration phase. For more information, refer to [Edge Host Registration](../../site-deployment/site-installation/edge-host-registration.md).    |
