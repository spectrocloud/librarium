...
- The TUI does not support changing the root user's password. If the root password has expired, update it from the terminal using a privileged user account, for example, with `sudo passwd root`.
