...
| GPU          | The Graphics Processing Unit (GPU) information of the Edge host, including the GPU model, vendor, memory, count, and Multi-Instance GPU (MIG) capability and strategy. The MIG fields are applicable only for Nvidia devices. The Nvidia System Management Interface (`nvidia-smi`) CLI must be installed on the Edge host to retrieve GPU information. | No               |
