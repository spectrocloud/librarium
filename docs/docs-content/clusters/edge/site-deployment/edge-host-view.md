...
| GPU          | The Graphics Processing Unit (GPU) specifications of the Edge host, including the GPU model, vendor, memory, count, and Multi-Instance GPU (MIG) capability and strategy. If Palette cannot automatically retrieve the GPU information, you can provide it manually via either the `user-data` file (Appliance and Agent mode) or with a `custom-hardware-specs-lookup.json` file (Appliance mode only). Refer to [Prepare User Data and Argument Files](../edgeforge-workflow/prepare-user-data.md#configure-gpu-specifications-optional) for more information. | No               |
