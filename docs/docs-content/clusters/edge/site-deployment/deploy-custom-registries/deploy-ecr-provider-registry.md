...
This procedure applies to Edge clusters that use the PXK-E (kubeadm) Kubernetes distribution, and it covers provider images only. It does not configure ECR for application workload images, and it does not cover other Kubernetes distributions. Refer to
[Limitations](#limitations) for the full scope.
