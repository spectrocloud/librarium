...
9.  Fill out the following fields on the **Cluster Config** step. Select **Next** when finished.
    ...
    | **Host LXD-Based Control Planes**              | Activate to use this cluster as an LXD-based control plane (hypervisor) for running control plane components as LXD VMs. To create a workload cluster that leverages MAAS LXD or use an existing host LXD-based control plane, leave this option disabled. Refer to [Create and Manage MAAS Clusters Using LXD VMs](create-manage-maas-lxd-clusters.md) for more information on leveraging LXD in Palette. <br /> <br /> **WARNING:** LXD-based VMs on MAAS are supported only with Palette eXtended Kubernetes (PXK) clusters. Enabling this toggle on a cluster that uses a different Kubernetes distribution results in deployment failures. |
