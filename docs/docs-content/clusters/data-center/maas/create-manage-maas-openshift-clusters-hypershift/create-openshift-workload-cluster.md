...
8. Fill out the following fields on the **Cluster Config** step. Click **Next** when done.
   ...
   | **NTP Servers (Optional)**                     | _This field has no effect on OpenShift workload clusters_. Palette does not apply NTP servers to RHCOS worker nodes; time synchronization is governed by OpenShift and RHCOS mechanisms. Refer to [NTP on OpenShift Workload Clusters](../architecture.md#ntp-on-openshift-workload-clusters) for details.                                                                                                   |
