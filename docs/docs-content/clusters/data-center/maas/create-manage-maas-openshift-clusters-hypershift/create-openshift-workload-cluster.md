...
- The HyperShift host cluster created in [Create HyperShift Host Cluster](./create-hypershift-host-cluster.md) must be
  in **Running** state with the HyperShift Operator healthy.
...
- A cluster profile for your OpenShift workload clusters configured with the following packs:
  ...
  - <VersionedLink text="Bring Your Own Openshift OS (BYOOS)" url="/integrations/packs/?pack=openshift-byoi" /> pack for
    ...
    | ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------- |
    | `pack.osImageOverride` | The name of the MAAS image imported in [Build and Import MAAS-Compatible RHCOS Image](./build-import-rhcos-image.md). This should match the `<image-name>` parameter specified in the `import-maas-image.sh` script. | `rhcos-4.20.13-with-ubuntu` |
...
9. Select a **HyperShift host cluster** from the drop-down menu. This should match the name of the HyperShift host
   cluster you created in [Create HyperShift Host Cluster](./create-hypershift-host-cluster.md). This associates the
