...
perform actions such as creating, modifying, and deleting resources. These permissions allow Palette to interact with
the CloudStack API and manage the lifecycle of the clusters effectively.
...
These permissions must be granted to the CloudStack user account that you configure in Palette, which is the account
whose API key/secret you enter. This is typically a non-admin user account that has been assigned a custom role with the
