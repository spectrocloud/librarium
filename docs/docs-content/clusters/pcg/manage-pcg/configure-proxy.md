...
6. Open the **reach-system/values.yaml** file in a text editor. Fill out the following YAML fields with the proxy
   ...
   | `reachSystem.scheduleOnControlPlane`    | Specifies whether to schedule the reach system on the control plane. Due to node affinity configurations, you must set the `scheduleOnControlPlane: false` for managed clusters deployed to [Azure AKS](../../public-cloud/azure/aks.md), [AWS EKS](../../public-cloud/aws/eks.md) and [GCP GKE](../../public-cloud/gcp/create-gcp-gke-cluster.md). |
