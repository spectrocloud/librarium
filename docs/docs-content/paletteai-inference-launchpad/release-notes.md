...
- Deploys a model to the cluster after a guarded preview, gate, provision, and smoke-test sequence. On a multi-node
  appliance, you choose which nodes run the model. Refer to [Deploy a Model](./how-to-guides/deploy-a-model.md) and
  [Model Placement](./explanation/model-placement.md) for more information.
