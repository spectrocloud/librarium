...
description: >
  PaletteAI Inference Launchpad is a standalone, turnkey AI appliance that lets enterprises run large language models
  on-premises without cloud dependency, AI consulting, or complex infrastructure setup.
...
authentication, Role-Based Access Control (RBAC), and observability. PaletteAI Inference Launchpad delivers the whole
stack, pre-integrated, as a single bootable artifact.
...
- **Run your first model:** Follow the [Run Your First Model](/paletteai-inference-launchpad/tutorials/run-first-model)
  tutorial to deploy a model and send your first prompt.
