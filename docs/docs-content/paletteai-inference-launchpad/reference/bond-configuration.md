...
bond at one NIC's bandwidth. Confirm the switch is set to the same hash policy.
...
Palette TUI onto the bond when you apply the form, so the node keeps the same address, only on a different interface.
