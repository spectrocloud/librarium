...
`csi-vsphere-volume`, and `kubernetes`.
...
**Example**: `kubernetes` pack must be required of min version `1.17.0` and max version `1.18.6`. Any Kubernetes version
below `1.17.0` and above `1.18.6` is not valid.

</TabItem>

<TabItem label="notSupported" value="pack_dependency_notSupported">

Pack versions within the range of the mentioned minimum and maximum (including the minimum and maximum) are not
supported.
