...
[OCI registry](/registries-and-packs/registries/oci-registry/) section for more information.
...
- A Legacy Pack Registry server must be available and accessible from the CLI. Refer to the
  [Add Custom Registries](/registries-and-packs/adding-a-custom-registry/) section for more information.
...
```bash hideClipboard
Spectro CLI Version 4.5.0 linux/amd64
```
