...
3. **Download** the appropriate **Content bundle**, **Palette management appliance ISO**, or **Helm installation** that
   ...
   | **Appliance ISO with Content** | Full ISO with an embedded content bundle. Use to install on a virtual machine, edge device, or bare-metal system without a separate content bundle upload. A signature file (`sig.bin`) is included with the ISO file. |
