...
section. This will make the cluster profile pull the provider images from the registry you pushed them to. The image
below displays the OS layer with the custom manifest.
...
Per-host NTP values can differ between hosts. For example, one host may use `<ntp-val-1>` while another uses
`<ntp-val-2>`.

Cluster settings do not display previously configured per-host NTP values. They only display the cluster-wide
configuration.

When a Day-2 cluster settings update modifies NTP configuration, the new values are applied to all hosts in the cluster
