...
specific project with a per-call argument, and view a second cluster's clean status as a healthy-baseline comparison—one
that happens to be the same edge deployment from the [edge-triage tutorial](./edge-triage-palette-mcp.md), viewed from
the cloud side this time. Completing [Get Started with Palette MCP](./get-started-palette-mcp.md) first is assumed.
...
and its `uid`—capture the `uid` and `project_uid` for the cluster you want to triage; you'll need both in the next step.
...
This exact condition, on this exact cluster, was independently identified as the root cause during an earlier live
session weeks before this tutorial was written. Finding the same signature again, independently, is a strong sign the
skill's condition-routing table (Step 6) reliably points at the real problem.
...
that one request to a specific project.
...
`ImagePullSecretPropagationDone` is a status condition, not an event in this case. This is itself a useful result:
events confirm what already succeeded; they don't always surface what's currently failing. Read both conditions and
...
kubeconfig (never the admin one past the initial fetch) to inspect CAPI/node/pod state directly, and—for self-managed
infrastructure only (`aws`/`azure`/plain `gcp`, not managed `eks`/`aks`/`gke`)—can go one level further over SSH to a
node's own cloud-init/kubelet/containerd logs. That path creates and tears down real objects on the live cluster (a
...
like from this same tool. `diagnose-cluster`'s condition-routing table covers `aws`/`azure`/`gcp`/`eks`/`aks`/`gke`; for
an edge-native cluster's host-level triage, use the edge-triage tutorial's `run_edge_diagnostic` instead—two different
...
- If your triage escalates to kube-tier, a temporary ServiceAccount and ClusterRoleBindings are created on the live
  cluster and torn down at the end of the run—re-run cleanup manually if it reports failure.
