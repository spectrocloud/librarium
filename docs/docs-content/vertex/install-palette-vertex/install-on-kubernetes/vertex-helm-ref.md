...
Palette VerteX deploys both a Traefik Ingress Controller and an Nginx Ingress Controller. Traefik is the default ingress
controller and handles traffic routing to the VerteX control plane. Both controllers are always deployed, but the
`ingress.type` parameter determines which one serves as the primary ingress controller. You can omit the deployment of
both controllers by setting `ingress.enabled` to `false`.

| **Parameters**                         | **Description**                                                                                                                                                                                                                                                                                               | **Type** | **Default value** |
| -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ----------------- |
| `enabled`                              | Specifies whether to deploy ingress controllers. Set to `false` if you do not want ingress controllers deployed.                                                                                                                                                                                              | Boolean  | `true`            |
| `type`                                 | Specifies the type of ingress controller to use as the primary ingress. Options are `"nginx"` or `"traefik"`.                                                                                                                                                                                                 | String   | `"traefik"`       |
| `traefik.hostPort`                     | Specifies whether the Traefik Ingress Controller uses host ports. Set to `true` in appliance-based deployments where Traefik should bind directly to host ports 80 and 443.                                                                                                                                   | Boolean  | `false`           |
...
| `ingress.annotations`                  | A map of key-value pairs that specifies load balancer annotations for ingress. You can use annotations to change the behavior of the load balancer and the ingress configuration. This is an advanced setting. We recommend you consult with your assigned support team representative prior to modification. | Object   | `{}`              |
