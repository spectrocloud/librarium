...
Palette VerteX uses Traefik as the management cluster ingress controller. If your cluster already has an ingress
controller, set `ingress.enabled` to `false`.

| **Parameters**                         | **Description**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | **Type** | **Default value** |
| -------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ----------------- |
| `ingress.enabled`                      | Specifies whether to deploy the Traefik ingress controller. Set to `false` if you do not want Palette VerteX to deploy an ingress controller, such as if you have an existing ingress service configured on the Kubernetes cluster.                                                                                                                                                                                                                                                                                                                                                                                                                  | Boolean  | `true`            |
| `ingress.traefik.hostPort`             | Specifies whether the Traefik ingress controller uses host ports. When set to `true`, Traefik binds directly to ports 80 and 443 on the host nodes, and no `LoadBalancer` service is created. This setting is intended for environments without a cloud load balancer, such as [VerteX Management Appliance](../../install-palette-vertex/vertex-management-appliance.md) or [Palette CLI](../../install-palette-vertex/install-on-vmware/install-on-vmware.md) deployments. For [Helm chart installations](../../install-palette-vertex/install-on-kubernetes/install-on-kubernetes.md) on existing Kubernetes clusters, leave this set to `false`. | Boolean  | `false`           |
...
Palette VerteX can export platform metrics to an external observability stack, such as Splunk, using an OpenTelemetry
collector. The observability stack is disabled by default. Set `observability.enabled` to `true` at install or upgrade
time to enable it, then configure the export from the **Metrics** tab in the system console. For the configuration
