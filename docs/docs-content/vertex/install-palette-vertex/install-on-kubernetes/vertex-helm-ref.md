...
| **Parameters**       | **Description**                                                                                                                                                                                                                                                                                                                                                                                                   | **Type** | **Default value** |
| -------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ----------------- |
| `env.rootDomain`     | Specifies the URL name assigned to Palette VerteX. The value assigned should have a Domain Name System (DNS) CNAME record mapped to the exposed IP address or the load balancer URL of the `traefik-ingress-controller` service in the `ingress-traefik` namespace. Optionally, if `ingress.ingressStaticIP` is provided with a value you can use same assigned static IP address as the value to this parameter. | String   | `""`              |
...
| `grpcStaticIP`        | Specify a static IP address for the gRPC `LoadBalancer` service. If the field is empty, a dynamic IP address will be assigned to the load balancer.                                                                                                                                                                      | String   | `""`              |
...
Beginning with 4.8.47, Palette VerteX deploys both a Traefik ingress controller and an Nginx ingress controller. Traefik
is the default ingress controller and handles traffic routing to the Palette VerteX control plane. Ingress Nginx acts as
a fallback and does not actively serve traffic. The `ingress.type` parameter determines which one serves as the primary
ingress controller. If your cluster already uses an ingress controller, set `ingress.enabled` to `false`.

:::warning

Ingress Nginx is a [deprecated Kubernetes project](https://www.kubernetes.dev/blog/2025/11/12/ingress-nginx-retirement/)
and is also [deprecated in Palette VerteX](../../../release-notes/announcements.md#deprecations). It will be removed in
a future release. To avoid service disruptions, set `ingress.type` to `traefik`.

:::

| **Parameters**                         | **Description**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | **Type** | **Default value** |
| -------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ----------------- |
| `enabled`                              | Specifies whether to deploy ingress controllers. Set to `false` if you do not want ingress controllers deployed, such as if you have an existing ingress service configured on an existing Kubernetes cluster you are installing Palette VerteX on.                                                                                                                                                                                                                                                                                                                                                                                                  | Boolean  | `true`            |
| `type`                                 | Specifies the type of ingress controller to use as the primary ingress. Options are `"nginx"` or `"traefik"`. <br /> <br /> **WARNING:** Ingress Nginx is a [deprecated Kubernetes project](https://www.kubernetes.dev/blog/2025/11/12/ingress-nginx-retirement/) and is also [deprecated in Palette VerteX](../../../release-notes/announcements.md#deprecations). It will be removed in a future release. To avoid service disruptions, set `ingress.type` to `traefik`.                                                                                                                                                                           | String   | `"traefik"`       |
| `traefik.hostPort`                     | Specifies whether the Traefik ingress controller uses host ports. When set to `true`, Traefik binds directly to ports 80 and 443 on the host nodes, and no `LoadBalancer` service is created. This setting is intended for environments without a cloud load balancer, such as [VerteX Management Appliance](../../install-palette-vertex/vertex-management-appliance.md) or [Palette CLI](../../install-palette-vertex/install-on-vmware/install-on-vmware.md) deployments. For [Helm chart installations](../../install-palette-vertex/install-on-kubernetes/install-on-kubernetes.md) on existing Kubernetes clusters, leave this set to `false`. | Boolean  | `false`           |
| `ingress.internal`                     | Specifies whether to use host networking instead of a cloud load balancer for the ingress controller. When set to `false`, a `LoadBalancer` service is created to expose the ingress controller. When set to `true`, the ingress controller binds directly to the host network and no `LoadBalancer` service is created.                                                                                                                                                                                                                                                                                                                             | Boolean  | `false`           |
| `ingress.certificate`                  | Specify the base64-encoded x509 SSL certificate for the ingress controller. If left blank, a self-signed certificate is generated.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | String   | `""`              |
| `ingress.key`                          | Specify the base64-encoded x509 SSL certificate key for the ingress controller.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | String   | `""`              |
| `ingress.annotations`                  | A map of key-value pairs that specifies load balancer annotations for the ingress service. Annotations control cloud-provider-specific load balancer behavior such as internal routing, SSL certificates, and backend protocols. Misconfigured annotations can make the Palette VerteX management plane unreachable. We recommend consulting with our Support team before modifying these values.                                                                                                                                                                                                                                                    | Object   | `{}`              |
| `ingress.ingressStaticIP`              | Specify a static IP address for the ingress `LoadBalancer` service. If empty, a dynamic IP address is assigned to the load balancer.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | String   | `""`              |
| `ingress.terminateHTTPSAtLoadBalancer` | Specifies whether the load balancer terminates Hypertext Transfer Protocol Secure (HTTPS) connections. When set to `false`, encrypted traffic passes through the load balancer to the ingress controller, which handles TLS decryption. When set to `true`, the load balancer decrypts HTTPS traffic and forwards unencrypted Hypertext Transfer Protocol (HTTP) connections to the ingress controller. This requires the load balancer to have a valid TLS certificate, which you can configure through `ingress.annotations`.                                                                                                                      | Boolean  | `false`           |
