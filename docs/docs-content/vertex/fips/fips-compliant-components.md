...
| Traefik Ingress Controller | Traefik is the default ingress controller starting with Palette VerteX 4.8.c. It provides reverse proxy, configurable traffic routing, and Transport Layer Security (TLS) termination for Kubernetes services. Traefik uses Custom Resource Definitions (CRDs) for routing configuration. |
