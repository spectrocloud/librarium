...
- If you installed Palette VerteX on [Kubernetes](../install-palette-vertex/install-on-kubernetes/install-on-kubernetes.md), ensure that you created a certificate for the custom domain name you specified for your Palette VerteX installation. If you installed Palette VerteX on [VMware](../install-palette-vertex/install-on-vmware/install-on-vmware.md), or did not specify a custom domain name, you must create a certificate for the Palette VerteX system console’s IP address. You can also specify a load balancer IP address if you are using a load balancer to access Palette VerteX.
