...
description: >
  Reference table that maps each supported NVIDIA and AMD GPU configuration to the LLM models certified for Launchpad
  for AI.
...
Spectro Cloud certifies a small set of large language models (LLMs) for the hardware that Launchpad for AI supports. A
_certified_ model is one that Spectro Cloud has tested on the listed GPU configuration. For what certification covers,
refer to [Model Certification](../explanation/model-certification.md).
