...
description:
  "Step-by-step guidance for platform operators on how to deploy an LLM model to a running Launchpad for AI appliance
  and verify that it is serving."
...
This guide explains how to deploy a model to a running Launchpad for AI appliance and verify that the model is serving
requests. The appliance places the model automatically on the best-fit node. For how placement and the deploy lifecycle
work, refer to [Model Provisioning Lifecycle](../explanation/architecture.md#model-provisioning-lifecycle).
