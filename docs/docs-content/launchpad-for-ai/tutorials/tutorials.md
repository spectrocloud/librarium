---
sidebar_label: "Tutorials"
title: "Launchpad for AI Tutorials"
description: "Hands-on, end-to-end tutorials for deploying and running Launchpad for AI."
hide_table_of_contents: false
sidebar_position: 0
tags: ["launchpad-for-ai", "tutorial"]
---

Tutorials teach you a new skill by guiding you through a complete, working example from start to finish.
Each tutorial ends with a working result you can build on.

## Contents

| **Tutorial**                    | **What you do**                                                                         |
| ------------------------------- | --------------------------------------------------------------------------------------- |
| [Get Started](./get-started.md) | Deploy the appliance, form a cluster, load a model, and run your first inference query. |
