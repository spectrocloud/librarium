...
always creates a Kubernetes Secret named `spectro-image-pull-secret` from this value and distributes it to the
...
and it is only required for airgapped deployments or when you use a self-hosted OCI registry. You must also install the
Palette Image Swap Helm chart to use this option, otherwise, Palette will ignore the configuration.
...
Palette uses Traefik as the management cluster ingress controller. If your cluster already has an ingress controller,
set `ingress.enabled` to `false`.

| **Parameters**                         | **Description**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | **Type** | **Default value** |
| -------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ----------------- |
| `ingress.enabled`                      | Specifies whether to deploy the Traefik ingress controller. Set to `false` if you do not want Palette to deploy an ingress controller, such as if you have an existing ingress service configured on the Kubernetes cluster.                                                                                                                                                                                                                                                                                                                                                                                                      | Boolean  | `true`            |
| `ingress.traefik.hostPort`             | Specifies whether the Traefik ingress controller uses host ports. When set to `true`, Traefik binds directly to ports 80 and 443 on the host nodes, and no `LoadBalancer` service is created. This setting is intended for environments without a cloud load balancer, such as [Palette Management Appliance](../../install-palette/palette-management-appliance.md) or [Palette CLI](../../install-palette/install-on-vmware/install-on-vmware.md) deployments. For [Helm chart installations](../../install-palette/install-on-kubernetes/install-on-kubernetes.md) on existing Kubernetes clusters, leave this set to `false`. | Boolean  | `false`           |
...
Palette can export a curated set of platform metrics to an external observability stack, such as Splunk, using an
