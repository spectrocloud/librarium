...
default ingress controller and handles traffic routing to the Palette control plane. Both controllers are always
deployed, but the `ingress.type` parameter determines which one serves as the primary ingress controller. You can
prevent deploying both controllers by setting `ingress.enabled` to `false`.
...
| `traefik.hostPort`                     | Specifies whether the Traefik Ingress Controller uses host ports. Set to `true` in appliance-based deployments where Traefik should bind directly to host ports 80 and 443. <!-- ONLY APPLIANCE DEPLOYMENTS? -->                                                       | Boolean  | `false`           |
| `ingress.internal`                     | Specifies whether to deploy a load balancer or use the host network. <!-- WHAT DOES FALSE MEAN? LOAD OR HOST?-->                                                                                                                                                       | Boolean  | `false`           |
...
| `ingress.annotations`                  | A map of key-value pairs that specifies load balancer annotations the for ingress. You can use annotations to change the behavior of the load balancer and the ingress configuration. We recommending consulting with our Support team before modifying <!-- WHY? -->. | Object   | `{}`              |
...
| `ingress.terminateHTTPSAtLoadBalancer` | Specifies whether to terminate Hypertext Transfer Protocol Secure (HTTPS) at the load balancer. <!-- I SEE THIS PHRASE A LOT BUT DON'T ACTUALLY KNOW WHAT IT MEANS>                                                                                                    | Boolean  | `false`           |
