...
19. The last step is to start setting up a tenant. To learn how to create a tenant, check out the
    ...
    If Palette has only one tenant and you use local accounts with Single Sign-On (SSO) disabled, you can access Palette using
    the IP address or any domain name that resolves to that IP. However, once you enable SSO, users must log in using
