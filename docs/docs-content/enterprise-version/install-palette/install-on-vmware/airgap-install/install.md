...
8.  When prompted, enter the information listed in each of the following tables.
    ...
    | **No Proxy**                      | (Optional) If using an HTTP or HTTPS proxy, provide a list of local network Classless Inter-Domain Routing (CIDR) addresses, hostnames, and domain names to exclude from being proxied. This setting propagates to all nodes to bypass the proxy server. Example if you have a self-hosted environment: `my.company.com,10.10.0.0/16`.                           |
