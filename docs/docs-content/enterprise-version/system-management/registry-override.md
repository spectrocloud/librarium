...
5. Next, update the`ociImageRegistry` section with your OCI registry values. Refer to the table below for a description
   ...
   | `mirrorRegistries`   | [image swap format](https://github.com/phenixblue/imageswap-webhook/blob/master/docs/configuration.md) for pulling images. Ensure a `/v2/` endpoint is used if your OCI registry supports the Docker Registry protocol v2; otherwise, container pull images will fail. For example: `docker.io::harbor.example.org/v2/airgap-images/docker.io,gcr.io::harbor.example.org/v2/airgap-images/gcr.io` | Yes      |
...
9. Replace the placeholder values with the actual values for your custom image registry. Use the same values that you
   ...
   | `MIRROR_REGISTRIES` | A comma-separated list of mirror registries in the [image swap format](https://github.com/phenixblue/imageswap-webhook/blob/master/docs/configuration.md) to use for pulling images. Ensure a `/v2/` endpoint is used if your OCI registry supports the Docker Registry protocol v2; otherwise, container pull images will fail. For example: `docker.io::harbor.example.org/v2/airgap-images/docker.io,gcr.io::harbor.example.org/v2/airgap-images/gcr.io` | Yes      |
