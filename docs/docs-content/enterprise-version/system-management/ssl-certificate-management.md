...
- If you installed Palette on [Kubernetes](../install-palette/install-on-kubernetes/install-on-kubernetes.md), ensure that you created a certificate for the custom domain name you specified for your Palette installation. If you installed Palette on [VMware](../install-palette/install-on-vmware/install-on-vmware.md), or did not specify a custom domain name, you must create a certificate for the Palette system console’s IP address. You can also specify a load balancer IP address if you are using a load balancer to access Palette.
