...
title: "Install Airgap Palette on VMware vSphere with Palette CLI"
...
8.  When prompted, enter the information listed in each of the following tables.
    ...
    | **Proxy CA Certificate Filepath** | (Optional) If using an HTTP or HTTPS proxy, provide the filepath of the proxy's Certificate Authority (CA) certificate on the installer host. The certificate is copied to each node when deploying the EC, and the provided path is used on the EC nodes. The CA certificate file _must_ be named `ca.crt`. Example: `/usr/local/share/ca-certificates/ca.crt`. |
