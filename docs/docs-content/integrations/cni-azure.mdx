...
- Windows node pools are not supported for AKS clusters using a Cilium network dataplane (when `networkDataplane: cilium` is set in the Azure CNI pack values). Users requiring Cilium must select an **OS Type** of **Linux** when deploying node pools.
