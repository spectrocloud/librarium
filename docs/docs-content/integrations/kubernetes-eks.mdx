...
Palette reconciles the roles defined in `irsaRoles` throughout the cluster lifetime. Edits to `irsaRoles` in the cluster
profile take effect on the next reconcile: policies are attached or detached, service account changes update the trust
policy, and removing an entry deletes the underlying IAM role. When you delete an EKS cluster, Palette deletes every IAM
role it manages for that cluster.

Palette only manages the IAM roles it created for the cluster and does not modify roles it does not own. If a role with
the same name already exists in AWS when you add it to `irsaRoles`, Palette adopts it when its policies and trust policy
match the profile, and otherwise leaves it untouched and reports a warning on the cluster. Refer to
[IRSA reconcile warnings](../troubleshooting/cluster-deployment.md#scenario---irsa-role-reconcile-warning) to troubleshoot warnings.
...
apply when you have selected a custom
...
the [Configure Amazon Linux 2023 AMI Nodes](#configure-amazon-linux-2023-ami-nodes) section.

:::

### Day-2 Usage Considerations

Any Day-2 additions or changes to the `nodeCustomization` section will invoke a
...
| [Configure IAM Roles for Service Accounts](#configure-iam-roles-for-service-accounts)                               | `iam:CreateRole` <br /> `iam:AttachRolePolicy` <br /> `iam:PassRole` <br /> `iam:GetRole` <br /> `iam:ListRoles` <br /> `iam:ListRolePolicies`                                                                                                            |
...
If using the [Core IAM Policies](../clusters/public-cloud/aws/required-iam-policies/core-iam-policies.md), including the
[Controllers EKS Policy](../clusters/public-cloud/aws/required-iam-policies/additional-iam-policies-specific-use-cases.md#controllers-eks-policy), you may need the
...
If using
[Minimum Permissions (dynamic or static)](../clusters/public-cloud/aws/required-iam-policies/minimum-permissions-policies.md),
including the [Controllers EKS Policy](../clusters/public-cloud/aws/required-iam-policies/additional-iam-policies-specific-use-cases.md#controllers-eks-policy),
