...
| `providerCredentials.registry` | Specifies the private registry for the cluster to pull images from. If you are using a Harbor registry, provide the Harbor domain as well as the name of the project. For example, `harbor.spectrocloud.com/default`. If you are using a Docker registry, provide the domain of the registry. For example, `registry-1.docker.io`. | string |  
