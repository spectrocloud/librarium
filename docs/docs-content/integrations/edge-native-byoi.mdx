...
You can also set the following parameters in your pack YAML file.
...
| `podSelector` | Specify pod selector options for draining specific pods | ?? | N/A |
...
| Parameter | Description | Type | Default |
| --------- | ----------- | ---- | ------- |
| `registry`  | The registry location for your provider images. This is prepended to the value in your `system.uri` setting.  | string | `example.com/spectro-images` | 
...
# adding yip stages in os pack is the same as adding yip stages in the userdata.
stages:
