...
Customers may want to configure a pod CIDR to maintain predictable, scalable, and conflict-free networking within their Kubernetes clusters. By selecting a custom pod CIDR range, they can ensure alignment with existing network policies, avoid overlaps with corporate or cloud VPC networks, and plan for future cluster growth.
...
Once the above resources are created, you can proceed with the deployment of the CNI pack.

1. Provide your AWS account ID to all `account:` fields.

2. Change the following environment variables in the `env` section of the pack. Refer to the [AWS VPC CNI documentation](https://github.com/aws/amazon-vpc-cni-k8s#cni-configuration-variables) for more details. 
...
3. Provide the subnet IDs and security group IDs in the `eniConfig` section of the pack. Replace the placeholder values with your created subnet and security group IDs.
