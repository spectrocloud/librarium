...
5. Provide the version number as a [Semantic Versioning](https://semver.org) value, for example `1.2.3` or `1.2.3-rc.1`.
   Short forms and a leading lowercase `v` are also accepted and are normalized (for example,
   `1.2 is expanded to `1.2.0`and`v1.2.3`becomes`1.2.3`. Palette rejects malformed strings such as `2.2.2.develop`or`V1.2`(with an uppercase`V`).
