...
4. Provide the following basic information for your app profile and click **Next**.
   ...
   | **Version**          | An optional version number for the new app profile. You only need to specify a version if you create multiple versions of a profile using the same profile name. The value must be a valid [Semantic Versioning](https://semver.org) string, for example `1.2.3` or `1.2.3-rc.1`; short forms such as `1.2` are expanded as `1.2.0`. Palette rejects malformed strings such as `2.2.2.develop` or `b0.0.1`. Default: `1.0.0`. |
