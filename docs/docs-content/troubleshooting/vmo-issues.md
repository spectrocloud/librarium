...
blocks the VMO GUI from loading when your Palette instance uses a different domain name.
...
8. Within a few minutes, the VMs will stop being stuck in the continuous migration loop.
   ...
   Step **4** affects the [CPU Hotplug](../vm-management/vmo-pack/create-manage-vm/enable-cpu-hotplug.md) functionality.
   If your cluster has CPU Hotplug enabled, you will need to manually restart your VM each time you add a virtual CPU
