...
steps to below to verify the error was caused by inconsistency in the primary registry and fix the issue.

### Debug Steps

1. Issue the following command and observe if there are files that are 0 byte in size.

   ```bash
   ls -la /usr/local/spectrocloud/peerbundle/sha256/
   ```

   If you observe output similar to the following, where the size of the file is zero, proceed to the next step.
