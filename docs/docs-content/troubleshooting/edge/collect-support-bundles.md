...
2. Download the `support-bundle-edge.sh` script and run it as a user with root privileges.
   ...
   If the script runs successfully, it creates a file in the directory specified by the `-d` flag. If you do not specify `-d`, the script saves the file in the system's temporary directory (typically `/tmp`). The file name follows the pattern `<hostname>-<YYYY-MM-DD>_<HH_MM_SS>.tar.gz`. One of the messages the terminal displays contains the created file's name and its path.

   ```bash
   Logs are archived in /tmp/tmp.qrKKeY1LqA/spectrocloud/logs/edge-e965384209c2d45078a29480e90bd275-2025-05-13_16_58_20.tar.gz
   ```
