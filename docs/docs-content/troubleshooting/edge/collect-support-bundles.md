...
2. Download and run the `support-bundle-infra.sh` script.
   ...
   If the script runs successfully, it creates a file in the directory specified by the `-d` flag. If you do not specify `-d`, the script saves the file in your home directory by default.
...
2.  Download the `support-bundle-edge.sh` script and run it as a user with root privileges.
    ...
    If the script runs successfully, it creates a file in the directory specified by the `-d` flag. If you do not specify `-d`, the script saves the file in the system's temporary directory (typically `/tmp`). The file name follows
