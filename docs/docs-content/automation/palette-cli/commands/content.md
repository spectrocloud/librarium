...
|            | `--full-verify`    | Also verify the whole-file SHA-256 end to end. Per-chunk SHA-256 is always verified, so this adds a full pass over the source before the upload starts. The default is `false`.                                                             | bool   |
