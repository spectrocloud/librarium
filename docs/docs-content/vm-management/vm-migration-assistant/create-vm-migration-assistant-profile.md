...
- A healthy VMO cluster. Refer to the [Create a VMO Profile](../vmo-pack/create-vmo-profile.md) for further guidance.
...
5. Select **Add New Pack**. In the next window that displays, enter **Virtual Machine Migration Assistant** in the
   **Filter by name** search bar. The pack is in the **Public Repo** registry. Select the pack when it appears.
...
6. Palette displays the YAML file in the editor on the right. You can edit the YAML as needed. Review the following
   ...
   | `vm-migration-assistant-ui.console.deployment.env.userAuth`          | If you want to enable OIDC for the service console, set the value to `oidc`. The OIDC configuration is inherited from your Kubernetes pack configuration. The service console supports OIDC with **Palette** as the Identity Provider (IdP), or with a **Custom** third-party OIDC IdP, such as Okta or Azure Active Directory. If you use a Custom IdP, you must also configure the IdP redirect URI and refresh token. Refer to [Configure Custom OIDC for VM Migration Assistant](../rbac/configure_OIDC.md#configure-custom-oidc-for-vm-migration-assistant) for those steps, and to [SAML and OIDC SSO](../../user-management/saml-sso/saml-sso.md) for details on Palette OIDC. | `disabled`        |
