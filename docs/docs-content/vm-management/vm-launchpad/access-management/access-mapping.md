...
description: "Learn how to view resolved user and group access in PaletteAI VM Launchpad."
icon: " "
...
The **Access Mapping** page is a read-only view of every Kubernetes binding and VMO IAM mapping PaletteAI VM Launchpad
manages on your behalf. Use it to audit who has access to a given namespace and to trace which Kubernetes ClusterRole is
bound to a specific user or group, without opening a `kubectl` shell.
...
| **K8s Role**     | The Kubernetes ClusterRole the binding references. VM Launchpad marks built-in roles with a **ClusterRole** or **Role** pill.                                                           |
...
- **VMO IAM mappings.** A row with a value in **VMO Role** represents a mapping from the subject to a VMO platform role.
  This controls what the subject can view and do in the VM Launchpad UI.

- **Dual-plane grants.** Rows with values in both **K8s Role** and **VMO Role** are grants VM Launchpad created through
  the [Users](./users.md) or [Groups](./groups.md) pages, where a single action creates both the Kubernetes binding and
  the VMO IAM mapping.
...
The access chain that produces the bindings on this page depends on how VM Launchpad is deployed.

| **Mode**                    | **Description**                                                                                                                                                   |
| --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Keycloak**                | Direct OIDC with the appliance's built-in Keycloak. The browser redirects to Keycloak, receives tokens, and VM Launchpad creates a session.                       |
| **Palette or VerteX proxy** | The Palette or VerteX proxy sits between the browser and the identity provider. Tokens flow through the proxy before reaching VM Launchpad.                       |
| **Local**                   | No external identity provider. The browser sends credentials directly to VM Launchpad, and the session uses the pod's ServiceAccount token for Kubernetes access. |
