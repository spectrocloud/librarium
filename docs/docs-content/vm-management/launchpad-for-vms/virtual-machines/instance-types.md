...
- **CPU Model** — Optional. The named x86 CPU model exposed to the guest, such as `Skylake-Server` or `EPYC-Rome`, or a
...
The list of discovered CPU models is cached for up to three minutes. Adding, removing, or cordoning a worker node will
reflect in the dropdown within one cache window.
...
- **Machine type** — For example, `q35` or `pc-q35-rhel9.2.0`.
...
This ensures you do not apply a Windows preference, such as HyperV, to a Linux VM or vice versa.
...
- Create custom instance types for organization-specific sizing, such as `app-small`, `app-medium`, and `db-large`.

### Preferences

- **Workloads** > **Preferences** — List, create, edit, delete `VirtualMachineClusterPreference` resources.
- Create custom preferences for specific OS versions or security requirements, such as `rhel9-secure` or
