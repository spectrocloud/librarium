...
| `platform.baseUrl`               | Set this to the URL that users reach the UI at, so that VMO can build the OIDC redirect URI. In **Direct** mode, also keep `appConfig.clusterInfo.consoleBaseAddress` set to the same value to surface the **Connect** button. The pack plans to merge the two parameters in a later version. Refer to [Deployment Modes](./deployment-modes.md#direct).                                                                            |
...
| `appConfig.clusterInfo.consoleBaseAddress` | `vmo-manager.platform.baseUrl` for the OIDC redirect URI. In **Direct** mode, keep `appConfig.clusterInfo.consoleBaseAddress` set as well to surface the **Connect** button, and set both to the same value until the pack merges them. |
