...
[Go debugging and profiling interface (`debug/pprof`)](https://pkg.go.dev/net/http/pprof) may be accessible within
certain Stylus (Palette Edge agent) deployments. The exposed interface is intended for application debugging and
...
- Restrict network access to required Kubernetes control-plane components only.
...
Palette version 4.8.27 uses Nginx controller version 1.13.7.
