...
targets check the key before they ask you for anything. In a terminal, the script opens Super so that you can sign in,
waits for you to confirm, and then continues. A GitHub Actions runner has no browser, so the script stops with an error
instead. The owner of the key stored in the `SUPER_API_TOKEN` repository secret must sign in to Super before you run the
workflow again. A `generate-patch-release-notes` run that resolves no candidate issues never calls Super, so it skips
the check.
...
`NICKFURY_REF`, either component version variable, or any of the three checksum variables is supplied, which is taken to
mean the component versions are wanted. The **Generate Patch Release Notes** workflow presents the same values as form
fields, in the same order, and its component version tick box has to be set for the branch or tag, version, and checksum
fields to be used.
...
after the ref was cut can be replaced with the one you were given. The ref question only appears when the GitHub CLI is
set up, because without it there is nothing to read, and the version prompts appear either way.
...
The comparison is made against the newest row in the same release train, so a 4.9 patch is compared against the last 4.9
release rather than against the 4.10 row that sits above it in the table. That is also what `same` resolves to. The
newest row in the table is used only when that train has no rows yet, which is the first patch documented in a new
train. The Palette CLI install page is the exception, because it documents the version a reader should install rather
than one release's version: a pending marker there is repaired from the newest version in the CLI Tools table, whatever
train this patch belongs to, so that documenting a 4.9 patch cannot pull the page back to a 4.9 version.
...
- `make generate-patch-release-notes` creates patch release notes using the issue tracker API and Super, and records any
  ...
  issues, the version, the date, the component versions, and the checksums, and to remove the rows that the values it
