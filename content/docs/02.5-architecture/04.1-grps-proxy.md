...
| gRPC with HTTP/HTTPS - SSL bump | gRPC traffic is sent over HTTP/HTTPS using the `CONNECT` method, and the HTTP proxy performs an SSL bump. Support varies by vendor. | ⚠️ |
| gRPC with [Squid](https://wiki.squid-cache.org) Open Source HTTP Proxy | gRPC traffic is sent over HTTP/HTTPS using the `CONNECT` method, and the HTTP proxy performs an SSL bump. Supported in some scenarios but requires additional configuration. | ❌ or ⚠️ |
| gRPC with SOCKS Proxy| HTTP/2 traffic is not supported by SOCKS proxies. | ❌ | 
