...
<Tabs>
<Tabs.TabPane tab="4.0.0 or greater" key="gRPC">
...
- The Kubernetes cluster must have the following minimum resources:
  ...
  Refer to the Palette [size guidelines](/enterprise-version/on-prem-system-requirements#hardwarerequirements) resource for additional sizing information.
...
4. Open the **values.yaml** in the **spectro-mgmt-plane** folder with a text editor of your choice. The **values.yaml** contains the default values for the Palette installation parameters, however, you must populate the following parameters before installing Palette. 
    ...
    | `ociRegistry` or `ociEcrRegistry` | The OCI registry credentials for Palette FIPS packs.| object |
...
<Tabs.TabPane tab="3.4.0 or earlier" key="nats">
