...
Go ahead and review each of the five files in the pack.
<br />
...
2. **values.yaml** -  This file contains configurable parameters you can define while adding the current pack to a cluster profile. In the **values.yaml** file for this tutorial, the `pack/namespace` attribute specifies the namespace on the target cluster to deploy the pack. If the **values.yaml**  has a namespace value specified, then Palette will first check whether the namespace already exists. If present, Palette will use the existing namespace. If not, Palette will create a new namespace with the value specified. On the other hand, if the **values.yaml**  does not have a namespace value specified, Palette will deploy the application to the default namespace.
...
The previous command will reserve the current bash session and display the status of each HTTP request made to the Ngrok server later in this tutorial. The visual below shows the registry server successfully exposed via Ngrok.  
...
Login to the registry server using Ngrok's public URL assigned to you. Issue the following command and replace the URL with your Ngrok URL. 
<br/>
...
As a Tenant admin, navigate to **Admin Settings** > **Registries** > **Pack Registries** section. Click on **Add New Pack Registry** and enter the pack registry name, endpoint URL, and user credentials. For a consistent experience in this tutorial, use the name **private-pack-registry**, as shown in the visual below. Though you can choose any other name of your choice. Ensure you replace the URL with your Ngrok URL. Click on **Validate** to ensure the URL and credentials are correct, then click on **Confirm** to add the registry server.
...
1. **Cloud account**: A cloud account added to your Palette project settings. 
  ...
  The AWS cloud account name in this tutorial example is **spectro-cloud**. You can choose any name of your choice. The below visual shows how to add and verify AWS cloud account to your project by navigating to **Project Settings** > **Cloud Accounts** > **AWS** > **Add AWS Account** in Palette. Check out the [Register and Manage AWS Accounts](/clusters/public-cloud/aws/add-aws-accounts) guide for additional help. 
...
Once you confirm the core infrastructure layers, click the **Add New Pack** button at the top to add the add-on layers. 

As mentioned below, add the Spectro Proxy pack to enable a reverse proxy to connect to the cluster's API. Adding this pack is *optional*, but it will help connect your local machine to the cluster's API for debugging. 
Refer to the [Spectro Proxy](/integrations/frp/) for more details. 

|**Pack Type**|**Registry**|**Pack Name**|**Pack Version**| 
|---|---|---|---|
|Authentication | Public Repo| Spectro Proxy | `1.3.x`|

Add the following certificate Subject Alternative Name (SAN) value to the Kubernetes pack under the `apiServer` parameter section to configure the Spectro Proxy pack.
<br />
...
![Visual of the certificate Subject Alternative Name.](/tutorials/create-pack/registries-and-packs_create-pack_profile-certsan.png)

<br /> <br />

Click on the **Confirm & Create** button at the bottom to finish adding the Spectro Proxy pack. 

Next, add the following **Hello Universe** pack, the custom add-on pack you defined and pushed to the **private-pack-registry** earlier in this tutorial. 
...
Click on the **Confirm & Create** button at the bottom again to finish adding the Hello Universe pack. 
...
## Create a Cluster
From the **Profile** page,  click on the newly created cluster profile to view its details page. Palette will display the constituting layers and allow you to edit any of them. 
...
Keep the default values and click the **Next** button at the bottom to move on to the following section.    
...
- **inputs.tf** - contains the variables used in the tutorial, such as the names of cluster profile, cluster, cloud account, SSH key name, AWS region, pack name, and registry server. 
  ...
  Some variables have a default value, but you *must* provide the values for `cluster_cloud_account_aws_name`, `aws_region_name`, `ssh_key_name`, and `private_pack_registry` variables. You will find a `#ToDo` tag against each variable to update. Provide the values for these variables in the **terraform.tfvars** file. Leave the remaining variables as default.



- **terraform.tfvars** - contains the variable definitions. The list of variables is outlined in the code block below. You *must* specify the values for all variables having a `"REPLACE ME"` placeholder against them. Read the inline comments below to understand about each variable. 

  For example, the value for  `cluster_cloud_account_aws_name` will be the name of the cloud account added to your Palette project settings. In this tutorial example, the cloud account name is **spectro-cloud**. For `aws_region_name`, you can choose any [AWS region](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/Concepts.RegionsAndAvailabilityZones.html) for your deployment. This tutorial example uses **us-east-2** region. The value for `ssh_key_name` will be the name of the SSH key available in the region where you will deploy the cluster. The SSH key name used in this example is **aws_key_sk_us_east_2**. Lastly, you can provide the **private-pack-registry** as the value for the registry server if you have followed the same naming convention as this tutorial. 
  <br />
...
It will take up to 20 minutes to provision a cluster. After successful provisioning the cluster, you will see the following success message. 
<br />
...
The sections below provide the deletion steps using both Palette and Terraform. Use Palette user interface if you have used Palette to deploy the cluster; otherwise, use Terraform commands to delete the cluster. 
...
After deleting the cluster and the cluster profile, delete the registry service configuration from the Palette by navigating to **Admin Settings** > **Registries** > **Pack Registries** as a Tenant admin.
<br />

![Visual of registry server delete in Palette](/tutorials/create-pack/registries-and-packs_create-pack_registry-delete.png)

<br /> <br />

Lastly, stop the registry server by closing the tutorials container bash session serving the Ngrok reverse proxy server. At this point, you can close all bash sessions into the tutorials container. To remove the container and the image from the local machine, issue the following commands:
<br />
