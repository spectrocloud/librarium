...
To simplify this tutorial, we have provided you with the manifest file for the *hello-universe* application in the **packs/hello-universe-pack** sub-directory. Change the directory to the **packs/hello-universe-pack** folder.
<br />
...
5. **README.md** -  This file may contain the pack description, purpose, authors, and other relevant information. In this tutorial, we have used a minimal README outlining the application used in the pack.
<br />
...
The "tutorials" environment already has the Specktro registry service available. Follow the steps below to start and expose the registry server.
  ...
  - Next, expose the registry server to the public so that you can configure the registry server in Palette later. Use the [Ngrok](https://ngrok.com/) reverse proxy to expose the registry server listening on port 5000 via an HTTP tunnel using the following command. 
  ...
  - Verify the registry server is accessible from outside the tutorials container by visiting the `/health` endpoint. Access the *https://3735-106-215-88-147.ngrok-free.app/health*in your host browser. Replace the base of the URL with the output URL from the output you received.  You should receive a `{"status":"UP"}` response.
...
- Open up another bash session in the tutorials container from your local terminal. 
...
If you need to delete a pack, you can do so by using the following command. Replace the name of the pack with your pack's name.
<br />
...
After pushing the pack, configure the registry server in the Palette so that you can add the pack to cluster profiles.   

- Log in to the Palette console, and switch to the Tenant admin view.
...
Palettle syncs the registry server periodically. However, you can sync it manually by clicking the **three-dot Menu** against the registry server name and selecting the **Sync** option.  
...
## Create a Profile
Switch to the **Default** project scope and select the **Cluster Profile** section in the left **Main Menu** to create a cluster profile that combines core infrastructure and add-on layers. Click on the **Add Cluster Profile** button, and provide the details in the wizard that follows. The wizard will present you with several sections. 
<br />
...
To get started with the Terraform code, you need a Spectro Cloud API key to authenticate and interact with the Palette API endpoint. To add a new API key, log in to Palette, click on the user **User Menu** in the top-right corner, and select **My API Keys**, as shown in the visual below. 
...
To delete all the deployed resources, switch back to the tutorials container, and issue the following command from within the **terraform/pack-tf** directory:
<br />
...
We encourage you to check out the following resources to learn more about Palette.
