...
After deleting the cluster and the cluster profile, stop the registry server by closing the tutorials container bash session serving the Ngrok reverse proxy server. At this point, you can close all bash sessions into the tutorials container. To remove the container and the image from the local machine, issue the following commands:
<br />
