...
Navigate to **Tenant Settings** > **Registries** > **Pack Registries** section. Click on **Add New Pack Registry**. Palette will open a pop-up window asking for the fields to configure a custom pack registry, as highlighted in the screenshot below. 
<br />
...
For a consistent experience in this tutorial, we suggest using the name **private-pack-registry**. Use your Ngrok URL as the pack registry endpoint. Ensure to use an "https://" prefix in the pack registry endpoint. 

In the **TLS Configuration** section, select the **Insecure Skip TLS Verify** checkbox. This tutorial does not need a secure HTTPS connection between Palette and your pack registry server. Therefore, you can skip the TLS verification. However, in a production environment, should you need Palette to have a secure HTTPS connection while communicating with your pack registry server, you can upload your TLS certificate issued from a certificate authority (CA) for your pack registry server.

 
Click on **Validate** to ensure the URL and credentials are correct, then click on **Confirm** to add the registry server.
