...
- Spectro Cloud Palette has enhanced the import cluster functionality by adding a ["minimal permission"](/clusters/brownfield-clusters) mode in addition to the "full permission" mode. Users can start exploring Palette by importing a cluster in minimal model without granting the full administrative set of permissions. Over time, users can grant additional permissions to manage Day 2 operations.

- Palette now supports both Windows and Linux worker node in Azure Kubernetes Services (AKS).

- When using Ubuntu as an OS layer in many operating environments, users can leverage [Canonical's Ubuntu Advantage for Infrastructure](/integrations/ubuntu#ubuntuadvantage) subscription which can ensure security and OS patching.

- Scale your virtual machines instances automatically with [Autoscaler](/clusters/new-clusters/aks) when you set up the Microsoft Azure Kubernetes service and the Amazon Web Services Kubernetes clusters.
...
- The [Azure CNI pack](/integrations/azure-cni#azurecnipolicysupport) and [Calico Policies](/integrations/azure-cni#azureandcalicopoliciesandtheircapabilities) are available within Palette on AKS.
