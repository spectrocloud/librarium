...
5. Fill out the following input values and ensure you select **Add-on** for the type. Click on **Next** to continue.
  ...
  | **Type**| **Add-on** |
  | **Tags**| Assign any desired tags you want the profile to have. |
...
6. Select the type of layer to add to the cluster profile. 
  ...
  | **Manifest**| A manifest is a Kubernetes configuration file that describes the desired state of a Kubernetes resource, such as a deployment, service, or pod, and is used to create or modify that resource in a cluster.|




7. Depending on the type you selected, fill out the required input fields and click on **Confirm & Create**.
...
9. Select another type to create another layer or click on **Next** to review the profile.


10. Click on **Finish Configuration** to complete the cluster profile creation.



You now have an add-on cluster profile created. You can reuse the profile and apply it to serveral clusters. You can also update a cluster profile and decide what clusters to apply the new version to. Refer to the [Update Cluster Profile](/cluster-profiles/task-update-profile) guide for more information about update operations.
...
3. Select your cluster profile to review its layers or to make changes.
