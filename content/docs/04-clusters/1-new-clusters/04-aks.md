...
6. Complete the **Cluster config** section with the information for each parameter listed below.
    ...
    | **Placement**      | You may leave this unchecked, unless the choice of placement is Static, then select:<br /> <br /> **Resource Group for Virtual Network**: The select the virtual network from the dropdown menu. <br /><br /> **Virtual Network**: Controls the inbound or outbound network traffic from multiple Azure resources. <br /> <br /> **CIDR Block**: Enter the groups of addresses. <br /> <br /> **Control plane Subnet**: Select the control plane network from the dropdown menu. VNet Resource Group filtering allows the VNet to be a part of a different resource group than the AKS resource.
