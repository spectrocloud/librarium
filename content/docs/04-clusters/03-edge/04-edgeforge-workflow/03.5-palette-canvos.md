...
5. Review the **.arg** file containing the customizable arguments, such as image tag, image registry, image repository, and OS distribution. The table below shows all arguments, their default, and possible values.
  ...
  |`IMAGE_REGISTRY`|Image registry name|ttl.sh|Your image registry hostname, without `http` or `https` <br /> Example: docker.io/[docker-id]|
...
6. Edit the **.arg** file to customize the image tag for provider OS images, image registry, image repository, and OS distribution. Open up the **.arg** file in an editor and update the following variables. 
  <br />
    ...
    - Update the `CUSTOM_TAG` value to a string made up of lowercase alphanumeric characters. This example uses `palette-learn` value. 
    <br />
    ...
    - Update the `IMAGE_REGISTRY` value to point it to the Docker Hub host. If you use any other image registry, the hostname must comply with standard DNS rules but may not contain underscores. Replace `[docker-id]` in the declaration below with your Docker ID. If you do not have a Docker ID, check the prerequisites section and navigate to https://hub.docker.com to create one. 
    <br />

    ```bash
    IMAGE_REGISTRY=docker.io/[docker-id]    # Example: docker.io/spectrocloud
    ```

    - Update the `OS_DISTRIBUTION` value to use OpenSuse Leap. 
    <br />
  ...
  The final provider image name will have this syntax - `IMAGE_REGISTRY/OS_DISTRIBUTION:CUSTOM_TAG`. Ensure that this final artifact name must conform to the Docker Hub image name syntax - `[HOST[:PORT_NUMBER]]/[NAMESPACE]/[REPOSITORY]:[TAG]` to push your images to Docker Hub later in this guide. 
...
10. To use the provider OS images in your cluster profile, push them to your image registry. Issue the following command to login to Docker Hub. Provide your Docker ID and password when it prompts.
<br />
...
11. Use the following commands to push the the provider OS image to the Docker Hub image registry.  
<br />
