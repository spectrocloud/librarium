...
* An account with any image registry. This prerequisite is needed if you follow the Advanced workflow below. This guide uses the [Docker Hub](https://hub.docker.com/) registry as an example. If you do not have an account with Docker Hub already, refer to the [Create an account](https://docs.docker.com/docker-id/) page for signing-up instructions. You can use any other desired image registry to suit your requirements.
<br />

# Build Edge Artifacts

You can follow either of the workflows below - Basic or Advanced. Use Basic for a quick start so that you can get all the required artifacts for creating an Edge host cluster profile.  Use the Advanced section for detailed instructions and to learn about customization options you can apply to this workflow. 
<br />

<WarningBox>

You must have a tenant registration token before starting either of the workflows below.
...
5. Define a custom image tag for provider OS images. The current example uses `demo` as the default tag. To change the image tag, edit the value for the `CUSTOM_TAG` variable in the **.arg** file. Ensure that it is of lowercase alphanumeric characters. 
  <br />
...
8. To use the provider OS images in your cluster profile, push them to the image registry mentioned in the **.arg** file. The current example uses *[ttl.sh](https://ttl.sh/)* image registry. This image registry is free to use and does not require a sign-up. Images pushed to *ttl.sh* are ephemeral and will expire after the time limit. The default time limit is 24 hours, meaning images will be accessible for 24 hours. 

  Refer to the Advanced workflow in the current guide to learn how to use another registry, Docker Hub, and tag the docker images accordingly.

  Use the following commands to push the the provider OS image to the *ttl.sh* image registry.  
  <br />
  ...
  As a reminder, *ttl.sh* is a short-lived image registry. If you do not use these provider OS images in your cluster profile within 24 hours of pushing to *ttl.sh*, they will expire and must be re-pushed.
...
5. Edit the **.arg** file to customize the image tag for provider OS images, image registry, image repository, and OS distribution. 

  The table below shows all arguments defined in the **.arg** file, their default, and possible values.
  ...
  Open up the **.arg** file in an editor and update the following variables. 
  <br />
    ...
    - Update the value for the `OS_DISTRIBUTION` to OpenSuse Leap. This workflow supports OpenSuse Leap version 15.4.
    <br />
    ...
    - Create a repository in the Docker Hub named `opensuse-leap` and set the repository's default visibility to public. Refer to [Create a repository](https://docs.docker.com/docker-hub/repos/create/#create-a-repository) instructions for creating a repository in the Docker Hub. The following declaration will default the repository name to be same as the OS distribtion. 
    <br />
    ...
    You can choose any other repository name of your choice, if desired and update the `IMAGE_REPO` variable.     

    - The `OS_VERSION` variable is not applicable for OpenSuse Leap. Leave it as default. 

    - Use default values for the `K8S_DISTRIBUTION` and `ISO_NAME` variables. 
    <br />

  Using the customized arguments, the provider image name will be similar to `registry.hub.docker.com/opensuse-leap:k3s-1.25.2-v3.4.1-palette-learn`.
  <br />


6. Open up the Dockerfile in an editor, and modify the existing Dockerfile to suit your needs. For example, a sample Dockerfile to install [WireGuard](https://www.wireguard.com/install/) on the Edge host is available below. 
  <br />
...
7. Create a **user-data** file that embeds the Edge host's login credentials and registration token in the Edge Installer ISO image. Issue the command below to save your registration token in a local variable. 
<br />
...
8. Issue the following command to execute the **earthly.sh** file to build the Edge artifacts. The `--PE_VERSION` option in the command below signifies the Palette Edge version to use. Spectro Cloud generates a git tag for each new Palette release. The current example uses the **v3.3.3** git tag.
  <br />
...
9. To use the provider OS images in your cluster profile, push them to your image registry mentioned in the **.arg** file. In continuation to the current example, use the following commands to push the the provider OS image to the Docker Hub image registry.  
  <br />
  ...
  ```bash
  registry.hub.docker.com/opensuse-leap:k3s-1.24.6-v3.4.1-palette-learn
  ```
  <br />
