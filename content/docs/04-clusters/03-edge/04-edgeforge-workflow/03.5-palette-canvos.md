...
metaDescription: "Learn how to build Edge artifacts, such as the Edge Installer ISO and provider OS images using Spectro Cloud's CanvOS utility."
icon: ""
hideToC: false
fullWidth: false
...
* **Provider OS image** - This is a [Kairos](https://kairos.io/)-based image. This image installs an immutable Operating System (OS) and software dependencies compatible with a specific Kubernetes version at runtime, i.e., during the cluster deployment. A provider OS image is used in the OS layer while creating a cluster profile. 


In this guide, you will use Spectro Cloud's utility, [CanvOS](https://github.com/spectrocloud/CanvOS/blob/main/README.md), to build an Edge installer ISO image and multiple provider OS images for all the Palette-supported Kubernetes versions. It builds multiple provider OS images, so you can use either one that matches the Kubernetes version in your cluster profile. This guide assumes that your Palette supports K3s v1.24.6 and K3s v1.25.2, as an example, for deploying Edge clusters.  


The diagram below shows the high-level steps to building the Edge artifacts and pushing the provider OS images to an image registry.
...
The basic workflow has minimal customizations and offers a quick start to build Edge artifacts. This workflow builds an Ubuntu based Edge installer ISO and provider OS images. It will also push the provider OS images to the default image registry, [ttl.sh](https://ttl.sh/). 
...
10. The CanvOS utility uses [Earthly](https://earthly.dev/) to build the target artifacts. Issue the following command to invoke the Earthly build process. The `--PE_VERSION` option in the command below signifies the Palette product version to use, which refers to the git tag you checked out in the previous step. 
<br />
...
11. List the Docker images to review the provider OS images created. By default, provider images for all the Palette's Edge-supported Kubernetes versions are created. You can identify the provider images by reviewing the image tag value you used in the  **.arg** file's `CUSTOM_TAG` variable. 
<br />
...
13. After pushing the provider images to the image registry, switch to Palette to create a cluster profile, and add the [BYOS Edge OS](/integrations/byoos) pack to the OS layer. Add the following custom content within your OS layer configuration's `options` attribute so that the cluster profile can pull the provider OS image from the image registry. 
  ...
  ```yaml
    ...
    system.osVersion: 22
  ```
  The following screenshot illustrates how to incorporate custom content into an OS layer of a cluster profile using the BYOS Edge OS pack.
...
14. CanvOS utility uses [Earthly](https://earthly.dev/) to build the target artifacts. Issue the following command to invoke the Earthly build process. The `--PE_VERSION` option in the command below signifies the Palette product version to use, which refers to the git tag you checked out in the previous step.
<br />
...
15. List the Docker images to review the provider OS images created. By default, provider images for all the Palette's Edge-supported Kubernetes versions are created. You can identify the provider images by reviewing the image tag value you used in the  **.arg** file's `CUSTOM_TAG` variable. 
<br />
...
16. To use the provider OS images in your cluster profile, push them to your image registry mentioned in the **.arg** file. Issue the following command to log in to Docker Hub. Provide your Docker ID and password when it prompts.
<br />
...
After building the Edge artifacts, the next step is to use the Edge Installer ISO image to prepare your Edge host. Check out the [Prepare Edge Host for Installation](/clusters/edge/site-deployment/stage) guide to learn more about the next steps.