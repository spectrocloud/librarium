...
Palette's Edge native solution requires Edge hosts with [Kairos](https://kairos.io/) installed, allowing you to use the immutable Linux distribution of your choice. In this how-to guide, you will use Spectro Cloud's utility, CanvOS, which internally uses Kairos and other tools to create the following Edge native artifacts:
<br/>

* **An Edge installer ISO image** - This ISO image includes the Edge Installer, which installs the Palette agent and the operating system in the Edge host. 


* **Provider OS images** - The provider images are all the required container images to support your Edge environments, such as Operating System (OS), Kubernetes distribution, and other software dependencies.
...
# Overview
The instructions in this guide are split into the following subsections:
