...
The diagram below shows the high-level steps to build the artifacts and push to an image registry.
...
The advanced workflow offers more customizations and will help you learn other possible customization options. This workflow builds an OpenSuse based Edge installer ISO and provider OS images. It will push the provider OS images to your Docker Hub image registry. 
...
<br />
  ...
  Issue the following command to check if you can create privileged containers on your Linux machine.
  <br />

  ```bash
  docker run --privileged hello-world
  ```
  If the command above fails, create a `docker` user group and add the current user to it using the instructions available [here](https://docs.docker.com/engine/install/linux-postinstall/). Alternatively, if you do not want to create a user group, ensure you can issue Docker commands with `sudo` privileges on your Linux machine.
...
4. Check out the repository tag that matches your Palette product version. Your Palette product version is displayed in the top left corner, as shown in the screenshot below. 
  ...
  It is essential to check out a matching git tag so the generated Edge installer will be compatible with your Palette product version. This guide uses **v3.4.3** tag as an example. 
  <br />
...
5. Review the files relevant for this guide. 
    - **.arg** - Defines arguments to use during the build process. 
...
6. Open the **.arg** file in an editor to change one of the arguments. 
<br />
...
7. In the **.arg** file, change the value of the `CUSTOM_TAG` variable to any lowercase and alphanumeric string. This variable defines the custom image tag for the provider OS images. This guide uses the following custom tag as an example. 
<br /> 
...
8. Issue the command below to save your registration token to a local variable. 
<br />
...
9. Copy and issue the command below to create the **user-data** file that includes the tenant registration token. Also, you can click on the *Points of Interest* numbers below to learn more about the main attributes relevant to this example. 
  <br />
...
10. CanvOS utility internally uses [Earthly](https://earthly.dev/) to build the target artifacts. Issue the following command to invoke the Earthly build. The `--PE_VERSION` option in the command below signifies the Palette Edge version to use, which refers to the git tag you checked out in the previous step. 
<br />
  <WarningBox>

  Ensure to issue the command below with root-level or `sudo` privileges.
...
11. List the Docker images to verify the two provider OS images - one compatible with K3s v1.24.6 and another with K3s v1.25.2.
<br />
...
13. After pushing the provider images to the image registry, add the following custom content within the `options` attribute of your cluster profile's **OS layer** so that the cluster profile can pull the provider OS image from the image registry. 

  If you used customized arguments and Dockerfile to build the Edge artifacts, change these attributes' values, as applicable to you, before using them in your cluster profile.
  <br />
  ...
  ```yaml
    ...
    system.osVersion: 22
  ```
  The screenshot below shows custom content added to a cluster profile's OS layer.

  ![Screenshot of a sample cluster profile's OS layer ](/tutorials/palette-canvos/clusters_edge_palette-canvos_edit_profile.png)
  <br />

  After updating the OS layer, you must choose a Kubernetes version compatible with the provider OS image in your cluster profile. 
...
7. Open the **.arg** file in an editor to change the following  arguments.  
<br />
...
8. In the **.arg** file, change the value of the `CUSTOM_TAG` variable to any lowercase and alphanumeric string. This variable defines the custom image tag for the provider OS images. This guide uses the following custom tag as an example. 
    <br /> 
...
9. In the **.arg** file, update the `IMAGE_REGISTRY` value to point to your Docker Hub host or another image registry. Your image registry hostname must comply with standard DNS rules and may not contain underscores. Replace `[DOCKER-ID]` in the declaration below with your Docker ID. 
<br />
...
10. In the **.arg** file, update the `OS_DISTRIBUTION` value to use OpenSuse Leap. Use the default values for the remaining variables. 
<br />

  ```bash
  OS_DISTRIBUTION=opensuse-leap
  ``` 
  Save the **.arg** file, and exit the editor. 
...
11. Open the **Dockerfile** in a text editor.
<br /> 
...
13. Issue the command below to save your registration token to a local variable. 
<br />
...
14. Copy and issue the command below to create the **user-data** file. Also, you can click on the *Points of Interest* numbers below to learn more about the main attributes relevant to this example. 
  <br />
...
15. CanvOS utility internally uses [Earthly](https://earthly.dev/) to build the target artifacts. Issue the following command to invoke the Earthly build. The `--PE_VERSION` option in the command below signifies the Palette Edge version to use, which refers to the git tag you checked out in the previous step.
  <br />
...
16. List the Docker images to verify the two provider OS images - one compatible with K3s v1.24.6 and another with K3s v1.25.2.
<br />
...
17. To use the provider OS images in your cluster profile, push them to your image registry mentioned in the **.arg** file. Issue the following command to log in to Docker Hub. Provide your Docker ID and password when prompted.
<br />
...
18. Use the following commands to push the provider OS images to the Docker Hub image registry you specified. Replace the `[DOCKER-ID]` in the command below with your Docker ID. 
<br />
...
19. After pushing the provider images to the image registry, add the following custom content within the `options` attribute of your cluster profile's **OS layer** so that the cluster profile can pull the provider OS image from the image registry. 
