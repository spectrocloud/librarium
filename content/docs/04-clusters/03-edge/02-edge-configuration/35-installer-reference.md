...
## Custom UID Parameters

The UID is generated by a specific priority sequence. The below table outlines the priority order from top to bottom, to generate a UID.
...
Use the `regex` parameter to remove unsupported characters from attributes like product or board serial, ensuring a valid and compatible UID for seamless system integration. Refer [here]() for the list of unsupported characters.
...
The length of the UID truncates to a maximum allowed length of 128 characters.
