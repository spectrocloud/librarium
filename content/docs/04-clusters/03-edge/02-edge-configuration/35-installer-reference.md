...
| 4        | `Random UUID`                                    | Assigns a random UUID as the Edge host ID.     |
...
The length of the UID truncates to a maximum allowed length of 128 characters. Keep in mind that the following characters are unsupported: `/ ? # & + % , $ ~ ! @ * () {} | = ; : <> ' . ^ " `.
