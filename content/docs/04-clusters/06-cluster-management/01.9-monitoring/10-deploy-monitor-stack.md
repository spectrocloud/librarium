...
The Prometheus Operator pack will install an insecure Prometheus server. Refer to the [Enable Authentication and Encryption](/clusters/cluster-management/monitoring/authentication-encryption) guide to learn how to secure your monitoring stack.
...
If the Prometheus server is configured with authentication, add the authentication parameters. Replace `<USERNAME>` and `<PASSWORD>` with the actual credential values. Refer to the [Enable Authentication and Encryption](/clusters/cluster-management/monitoring/authentication-encryption) guide to learn more about authentication. 
